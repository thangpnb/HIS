...
using MPS.Processor.Mps000239.PDO;
...
{
    public class Mps000239Processor : AbstractProcessor
    {
        Mps000239PDO rdo;
        public Mps000239Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000239PDO)rdoBase;
        }
        ...
        public void SetBarcodeKey()
        {
            try
            {
                ...
                dicImage.Add(Mps000239ExtendSingleKey.EXP_MEST_CODE_BARCODE, expMestCodeBar);
            }
            catch (Exception ex)
            {
        ...
        void SetSingleKey()
        {
            try
            {
                if (rdo._ExpMests_Print != null && rdo._ExpMests_Print.Count > 0)
                {
                    ...
                    SetSingleKey(new KeyValue(Mps000239ExtendSingleKey.MIN_INTRUCTION_DATE_DISPLAY, Inventec.Common.DateTime.Convert.TimeNumberToDateString(minTime)));
                    SetSingleKey(new KeyValue(Mps000239ExtendSingleKey.MIN_INTRUCTION_TIME_DISPLAY, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(minTime)));
                    SetSingleKey(new KeyValue(Mps000239ExtendSingleKey.MIN_INTRUCTION_DATE_SEPARATE_DISPLAY, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(minTime)));

                    SetSingleKey(new KeyValue(Mps000239ExtendSingleKey.MAX_INTRUCTION_TIME_DISPLAY, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(maxTime)));
                    SetSingleKey(new KeyValue(Mps000239ExtendSingleKey.MAX_INTRUCTION_DATE_DISPLAY, Inventec.Common.DateTime.Convert.TimeNumberToDateString(maxTime)));
                    SetSingleKey(new KeyValue(Mps000239ExtendSingleKey.MAX_INTRUCTION_DATE_SEPARATE_DISPLAY, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(maxTime)));
                }

                AddObjectKeyIntoListkey<V_HIS_EXP_MEST>(rdo.AggrExpMest, false);
                AddObjectKeyIntoListkey<HIS_DEPARTMENT>(rdo.Department, false);

                SetSingleKey(new KeyValue(Mps000239ExtendSingleKey.CREATE_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(rdo.AggrExpMest.CREATE_TIME ?? 0)));
                SetSingleKey(new KeyValue(Mps000239ExtendSingleKey.CREATE_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo.AggrExpMest.CREATE_TIME ?? 0)));
                SetSingleKey(new KeyValue(Mps000239ExtendSingleKey.CREATE_DATE_SEPARATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(rdo.AggrExpMest.CREATE_TIME ?? 0)));

                SetSingleKey(new KeyValue(Mps000239ExtendSingleKey.TOTAL_REQ_ROOM_NAME_DISPLAY, totalReqRoomName));
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        void ProcessListADO()
        {
            try
            {
                ...
                if (rdo._ExpMestMedicines != null && rdo._ExpMestMedicines.Count > 0)
                {
                    ...
                    if (rdo.ConfigMps239._ConfigKeyMERGER_DATA == 1)
                    {
                        var Groups = query.GroupBy(g => new
                    {
                        ...
                        rdo.listAdo.AddRange(from r in Groups
                                             select new Mps000239ADO(rdo.AggrExpMest,
                                                 r,
                                                 rdo.ConfigMps239._ExpMestSttId__Approved,
                                                 rdo.ConfigMps239._ExpMestSttId__Exported,
                                                 rdo.ConfigMps239.PatientTypeId__BHYT));
                    }
                    else
                    {
                        var Groups = query.GroupBy(g => new
