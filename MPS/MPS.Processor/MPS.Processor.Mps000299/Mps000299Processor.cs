...
using MPS.Processor.Mps000299.PDO;
...
namespace MPS.Processor.Mps000299
{
    public class Mps000299Processor : AbstractProcessor
    {
        List<HisBillGoodsADO> serviceAdos = new List<HisBillGoodsADO>();
        Mps000299PDO rdo;
        public Mps000299Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000299PDO)rdoBase;
        }
        ...
        void ProcessSingleKey()
        {
            try
            {
                if (rdo.HisTransaction != null)
                {
                    ...
                    SetSingleKey(new KeyValue(Mps000299ExtendSingleKey.AMOUNT_TEXT_UPPER_FIRST, Inventec.Common.String.Convert.CurrencyToVneseString(amountStr)));
                    decimal amountAfterExem = rdo.HisTransaction.AMOUNT - (rdo.HisTransaction.EXEMPTION ?? 0);
                    SetSingleKey(new KeyValue(Mps000299ExtendSingleKey.AMOUNT_AFTER_EXEMPTION, Inventec.Common.Number.Convert.NumberToNumberRoundMax4(amountAfterExem)));
                    ...
                    SetSingleKey(new KeyValue(Mps000299ExtendSingleKey.AMOUNT_AFTER_EXEMPTION_TEXT, amountAfterExemText));
                    SetSingleKey(new KeyValue(Mps000299ExtendSingleKey.AMOUNT_AFTER_EXEMPTION_TEXT_UPPER_FIRST, Inventec.Common.String.Convert.UppercaseFirst(amountAfterExemText)));
                    ...
                    SetSingleKey(new KeyValue(Mps000299ExtendSingleKey.AMOUNT_AWAY_ZERO_TEXT_UPPER_FIRST, Inventec.Common.String.Convert.CurrencyToVneseString(amountAwayZeroStr)));
                }
            }
            catch (Exception ex)
            {
        ...
        public void SetBarcodeKey()
        {
            try
            {
                if (!String.IsNullOrEmpty(rdo.HisTransaction.TRANSACTION_CODE))
                {
                    ...
                    dicImage.Add(Mps000299ExtendSingleKey.TRANSACTION_CODE_BAR, barcodeTransactionCode);
                }
            }
            catch (Exception ex)
            {
