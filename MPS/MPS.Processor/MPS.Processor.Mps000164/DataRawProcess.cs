...
namespace MPS.Processor.Mps000164
{
    public class DataRawProcess
    {
        ...
        public static PatyAlterBhytADO PatyAlterBHYTRawToADO(HIS_PATIENT_TYPE_ALTER patyAlterBHYT)
        {
            ...
            try
            {
                if (patyAlterBHYT != null)
                {
                    ...
                    patyAlterBhytADO = AutoMapper.Mapper.Map<MOS.EFMODEL.DataModels.HIS_PATIENT_TYPE_ALTER, PatyAlterBhytADO>(patyAlterBHYT);

                    patyAlterBhytADO.HEIN_CARD_NUMBER_SEPARATE = SetHeinCardNumberDisplayByNumber(patyAlterBHYT.HEIN_CARD_NUMBER);
                    patyAlterBhytADO.HEIN_MEDI_ORG_CODE = patyAlterBHYT.HEIN_MEDI_ORG_CODE;
                    patyAlterBhytADO.HEIN_MEDI_ORG_NAME = patyAlterBHYT.HEIN_MEDI_ORG_NAME;
                    patyAlterBhytADO.IS_HEIN = "X";
                    patyAlterBhytADO.IS_VIENPHI = "";
                    if (!String.IsNullOrEmpty(patyAlterBHYT.HEIN_CARD_NUMBER))
                    {
                        patyAlterBhytADO.HEIN_CARD_NUMBER_1 = patyAlterBHYT.HEIN_CARD_NUMBER.Substring(0, 2);
                        patyAlterBhytADO.HEIN_CARD_NUMBER_2 = patyAlterBHYT.HEIN_CARD_NUMBER.Substring(2, 1);
                    ...
                    {
                        patyAlterBhytADO.STR_HEIN_CARD_FROM_TIME = Inventec.Common.DateTime.Convert.TimeNumberToDateString((patyAlterBHYT.HEIN_CARD_FROM_TIME.Value));
                    }
                    if (patyAlterBHYT.HEIN_CARD_TO_TIME.HasValue)
                    {
                        patyAlterBhytADO.STR_HEIN_CARD_TO_TIME = Inventec.Common.DateTime.Convert.TimeNumberToDateString((patyAlterBHYT.HEIN_CARD_TO_TIME.Value));
                    }
                }
                else
                {
        ...
        public static List<SereServGroupPlusADO> PriceBHYTSereServ(List<V_HIS_SERE_SERV> sereServs, HIS_PATIENT_TYPE_ALTER patyAlterBHYT)
        {
