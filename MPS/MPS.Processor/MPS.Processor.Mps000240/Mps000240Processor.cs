...
using MPS.Processor.Mps000240.PDO;
...
namespace MPS.Processor.Mps000240
{
    class Mps000240Processor : AbstractProcessor
    {
        Mps000240PDO rdo;
        List<Mps000240ADO> ImpMestManuMedicineSumForPrints = new List<Mps000240ADO>();
        public Mps000240Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000240PDO)rdoBase;
        }
        ...
        public void SetBarcodeKey()
        {
            try
            {
                ...
                dicImage.Add(Mps000240ExtendSingleKey.IMP_MEST_CODE_BAR, barcodePatientCode);
        ...
        void ProcessListADO()
        {
            try
            {
                ImpMestManuMedicineSumForPrints = new List<Mps000240ADO>();
                if (rdo._ImpMestMedicines != null && rdo._ImpMestMedicines.Count > 0)
                {
                    ...
                    ImpMestManuMedicineSumForPrints.AddRange((from r in dataGroups select new Mps000240ADO(r, rdo.AggrImpMest.IMP_MEST_STT_ID, rdo.HisImpMestSttId__Imported, rdo.HisImpMestSttId__Approved)).ToList());
                }
        ...
        void ProcessSingleKey()
        {
            try
            {
                if (rdo._MobaExpMests != null && rdo._MobaExpMests.Count > 0)
                {
                    ...
                    SetSingleKey(new KeyValue(Mps000240ExtendSingleKey.MIN_INTRUCTION_DATE_DISPLAY, Inventec.Common.DateTime.Convert.TimeNumberToDateString(minTime)));
                    SetSingleKey(new KeyValue(Mps000240ExtendSingleKey.MIN_INTRUCTION_TIME_DISPLAY, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(minTime)));
                    SetSingleKey(new KeyValue(Mps000240ExtendSingleKey.MAX_INTRUCTION_TIME_DISPLAY, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(maxTime)));
                    SetSingleKey(new KeyValue(Mps000240ExtendSingleKey.MAX_INTRUCTION_DATE_DISPLAY, Inventec.Common.DateTime.Convert.TimeNumberToDateString(maxTime)));
                }
                ...
                SetSingleKey(new KeyValue(Mps000240ExtendSingleKey.REQ_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(rdo.AggrImpMest.CREATE_TIME ?? 0)));
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
