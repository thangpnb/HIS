...
using MPS.Processor.Mps000370.PDO;
...
{
    public class Mps000370Processor : AbstractProcessor
    {
        Mps000370PDO rdo;
        List<HIS_SERVICE_TYPE> _ListServiceType = null;
        List<SereServDebtADO> _ListSereServDebtADO = null;
        public Mps000370Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000370PDO)rdoBase;
        }
        ...
        private void ProcessObjectTag()
        {
            try
            {
                _ListServiceType = new List<HIS_SERVICE_TYPE>();
                ...
                else if (rdo._listDebtGood != null && rdo._listDebtGood.Count > 0)
                {
                    if (rdo._listDebtGood.Exists(o => o.MEDICINE_TYPE_ID.HasValue))
                    {
                        var mety = rdo._listServiceType.FirstOrDefault(o => o.ID == IMSys.DbConfig.HIS_RS.HIS_SERVICE_TYPE.ID__THUOC);
                        if (mety != null)
                        {
                            _ListServiceType.Add(mety);
                        }
                    }

                    if (rdo._listDebtGood.Exists(o => o.MATERIAL_TYPE_ID.HasValue))
                    {
                        var mate = rdo._listServiceType.FirstOrDefault(o => o.ID == IMSys.DbConfig.HIS_RS.HIS_SERVICE_TYPE.ID__VT);
                        if (mate != null)
                        {
                            _ListServiceType.Add(mate);
                        }
                    }

                    if (rdo._listDebtGood.Exists(o => o.NONE_MEDI_SERVICE_ID.HasValue))
                    {
                        HIS_SERVICE_TYPE none = new HIS_SERVICE_TYPE();
                        none.ID = 0;
                        none.SERVICE_TYPE_CODE = "";
                        none.SERVICE_TYPE_NAME = "Dịch vụ khác";
                        _ListServiceType.Add(none);
                    }

                    foreach (var item in rdo._listDebtGood)
                    {
                        SereServDebtADO ado = new SereServDebtADO();
                        ado.TDL_AMOUNT = item.AMOUNT;
                        ado.CONCENTRA = item.CONCENTRA;
                        ado.DESCRIPTION = item.DESCRIPTION;
                        ado.DISCOUNT = item.DISCOUNT;
                        ado.EXPIRED_DATE = item.EXPIRED_DATE;
                        ado.TDL_SERVICE_NAME = item.GOODS_NAME;
                        ado.SERVICE_UNIT_NAME = item.GOODS_UNIT_NAME;
        ...
        private void SetSingleKey()
        {
            try
            {
                ...
                {
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.DOB_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo._Transaction.TDL_PATIENT_DOB ?? 0)));

                    string temp = rdo._Transaction.TDL_PATIENT_DOB.ToString();
                    if (temp != null && temp.Length >= 8)
                    {
                        SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.YEAR_STR, temp.Substring(0, 4)));
                    }
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.AGE_STR, AgeUtil.CalculateFullAge(rdo._Transaction.TDL_PATIENT_DOB ?? 0)));
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.AMOUNT_NUM, rdo._Transaction.AMOUNT));
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.AMOUNT, Inventec.Common.Number.Convert.NumberToString(rdo._Transaction.AMOUNT, HIS.Desktop.LocalStorage.ConfigApplication.ConfigApplications.NumberSeperator)));
                    ...
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.AMOUNT_TEXT, rdo._Transaction.AMOUNT));
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.AMOUNT_TEXT_UPPER_FIRST, amountText));
                    decimal amountAfterExem = rdo._Transaction.AMOUNT - (rdo._Transaction.EXEMPTION ?? 0);
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.AMOUNT_AFTER_EXEMPTION, Inventec.Common.Number.Convert.NumberToString(amountAfterExem, HIS.Desktop.LocalStorage.ConfigApplication.ConfigApplications.NumberSeperator)));
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.AMOUNT_AFTER_EXEMPTION_NUM, amountAfterExem));
                    ...
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.AMOUNT_AFTER_EXEMPTION_TEXT, amountAfterExemText));
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.AMOUNT_AFTER_EXEMPTION_TEXT_UPPER_FIRST, amountAfterExemText));
                    decimal ratio = ((rdo._Transaction.EXEMPTION ?? 0) * 100) / rdo._Transaction.AMOUNT;
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.EXEMPTION_RATIO, Inventec.Common.Number.Convert.NumberToNumberRoundMax4(ratio)));

                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.CREATE_DATE_SEPARATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(rdo._Transaction.CREATE_TIME ?? 0)));
                    AddObjectKeyIntoListkey<V_HIS_TRANSACTION>(rdo._Transaction, false);
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.DESCRIPTION, rdo._Transaction.DESCRIPTION));
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.EXEMPTION_REASON, rdo._Transaction.EXEMPTION_REASON));
                }
                ...
                if (rdo._ListSereServ != null && rdo._ListSereServ.Count > 0)
                {
                    ...
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.TOTAL_PRICE, Inventec.Common.Number.Convert.NumberToString(totalPrice, HIS.Desktop.LocalStorage.ConfigApplication.ConfigApplications.NumberSeperator)));
                }
                ...
                if (rdo._PatientTypeAlter != null)
                {
                    ...
                    if (ratio.HasValue)
                    {
                        SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.HEIN_RATIO_100, (1 - ratio.Value) * 100));
                    }
                    SetSingleKey(new KeyValue(Mps000370ExtendSingleKey.HEIN_CARD_ADDRESS, rdo._PatientTypeAlter.ADDRESS));
