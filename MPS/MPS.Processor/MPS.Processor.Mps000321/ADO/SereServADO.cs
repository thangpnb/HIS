...
namespace MPS.Processor.Mps000321.ADO
{
    public class SereServADO : HIS_SERE_SERV
    {
        ...
        public SereServADO(HIS_SERE_SERV data, List<HIS_SERE_SERV> SereServs, List<HIS_SERE_SERV_EXT> sereServExts, List<HIS_HEIN_SERVICE_TYPE> heinServiceTypes, List<V_HIS_SERVICE> services, List<V_HIS_ROOM> rooms, List<HIS_MATERIAL_TYPE> materialTypes, PatientTypeCFG patientTypeCFG, HisConfigValue hisConfigValue, List<HIS_SERVICE_UNIT> hisServiceUnit, List<HIS_PATIENT_TYPE_ALTER> ListPta, V_HIS_TREATMENT treatment)
        {
            try
            {
                #region Base
    ...
    || service.HEIN_SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_HEIN_SERVICE_TYPE.ID__TH_TL)
                ...
                if (String.IsNullOrWhiteSpace(this.KEY_PATY_ALTER) && ListPta.Count > 0)
                {
                    ...
                    this.KEY_PATY_ALTER = PatientTypeAlterProcessor.ToString(pta);
                ...
                if (this.TOTAL_PRICE_PATIENT_SELF > 0)
                {
                    ...
                    //if (this.PATIENT_TYPE_ID == patientTypeCFG.PATIENT_TYPE__FEE)
                    //{
                    //    if (this.PRIMARY_PATIENT_TYPE_ID.HasValue)
                    //    {
                    //        this.TOTAL_PRICE_PATIENT_SELF_TP = Math.Abs((this.LIMIT_PRICE ?? 0) - this.PRICE_BHYT);
                    //    }
                    //    else
                    ...
                    //else
                    //{
                    //    //chênh lệch giữa giá bhyt và trần bhyt
                    //    var diffPrice = this.HEIN_LIMIT_PRICE.HasValue ? Math.Abs(this.HEIN_LIMIT_PRICE.Value - this.PRICE_BHYT) : 0;
                    //    if (diffPrice != 0 && this.TOTAL_PRICE_PATIENT_SELF >= diffPrice)
                    //    {
                    //        this.TOTAL_PRICE_PATIENT_SELF_TP = diffPrice;
                    //    }
                    //    else
                    //    {
                    //        if (this.PRIMARY_PATIENT_TYPE_ID.HasValue)
                    ...
                    //        }
                    //        else
                    //            this.TOTAL_PRICE_PATIENT_SELF_TP = Math.Abs((this.PRIMARY_PRICE ?? 0) - this.PRICE_BHYT);
                    //    }

                    //    if (this.SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_SERVICE_TYPE.ID__G || this.SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_SERVICE_TYPE.ID__KH)
                    //    {
