...
namespace MPS.Processor.Mps000363.ADO
{
    class SereServADO : V_HIS_SERE_SERV
    {
        ...
        public SereServADO(V_HIS_SERE_SERV sereServADO, PDO.Mps000363PDO rdo)
        {
            if (sereServADO != null)
            {
        ...
        #region QRCODE
        private void ProcessQrCode(PDO.Mps000363PDO rdo)
        {
            try
            {
        ...
        private void ProcessQrDiim(PDO.Mps000363PDO rdo)
        {
            try
            {
        ...
        private void ProcessQrDiimV2(PDO.Mps000363PDO rdo)
        {
            try
            {
                List<string> qrInfos = new List<string>();
        ...
        private void ProcessQrCT540(PDO.Mps000363PDO rdo)
        {
            try
            {
                List<string> qrInfos = new List<string>();
        ...
        private void SereServSereServIdQrQR(PDO.Mps000363PDO rdo)
        {
            try
            {
                List<long> qrInfos = new List<long>();
                qrInfos.Add(this.ID);
                string totalQrInfo = string.Join("\t", qrInfos);
                QRCodeGenerator qrInfo = new QRCodeGenerator();
                QRCodeData qrInfoData = qrInfo.CreateQrCode(totalQrInfo, QRCodeGenerator.ECCLevel.Q);
                BitmapByteQRCode qrICode = new BitmapByteQRCode(qrInfoData);
                byte[] qrICodeImage = qrICode.GetGraphic(20);
                this.SereServIdQr = qrICodeImage;
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
