...
using MPS.Processor.Mps000485.PDO;
...
{
     class Mps000485Processor : AbstractProcessor
    {
        Mps000485PDO rdo;
        public Mps000485Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000485PDO)rdoBase;
        }
        ...
        public override bool ProcessData()
        {
            ...
            try
            {
                ...
                if (rdo.listEvents != null && rdo.listEvents.Count > 0)
                {
                    foreach (var item in rdo.listEvents)
                    {
                ...
                objectTag.AddObjectData(store, "EventsCauseDeathChain", lstPrint != null && lstPrint.Count > 0 && lstPrint.Where(o => o.IS_OTHER_CAUSE == 1).ToList() != null ? lstPrint.Where(o => o.IS_OTHER_CAUSE == 1).ToList() : new List<EventsCauseDeathADO>());
        ...
        public override string ProcessUniqueCodeData()
        {
            ...
            try
            {
                if (rdo != null && rdo.severeIllnessInfo != null && rdo.treatment != null)
                {
                    ...
                    result = String.Format("Mps000485 {0} {1}", treatmentCode, severeIllnessInfoId);
                }
            }
            catch (Exception ex)
            {
        ...
        void ProcessSingleKey()
        {
            try
            {
                ...
                if (rdo.treatment.TREATMENT_END_TYPE_ID != null)
                    SetSingleKey(new KeyValue(Mps000485ExtendSingleKey.TREATMENT_END_TYPE_NAME, rdo.treatmentEndType.FirstOrDefault(o => o.ID == rdo.treatment.TREATMENT_END_TYPE_ID).TREATMENT_END_TYPE_NAME));
                if (rdo.treatment.TREATMENT_RESULT_ID != null)
                    SetSingleKey(new KeyValue(Mps000485ExtendSingleKey.TREATMENT_RESULT_NAME, rdo.treatmentResult.FirstOrDefault(o => o.ID == rdo.treatment.TREATMENT_RESULT_ID).TREATMENT_RESULT_NAME));
                if (rdo.patientTypeAlter != null)
                {
                    SetSingleKey(new KeyValue(Mps000485ExtendSingleKey.HEIN_CARD_TO_TIME, rdo.patientTypeAlter.HEIN_CARD_TO_TIME));
                    SetSingleKey(new KeyValue(Mps000485ExtendSingleKey.HEIN_CARD_FROM_TIME, rdo.patientTypeAlter.HEIN_CARD_FROM_TIME));
                    SetSingleKey(new KeyValue(Mps000485ExtendSingleKey.JOIN_5_YEAR_TIME, rdo.patientTypeAlter.JOIN_5_YEAR_TIME));
                    SetSingleKey(new KeyValue(Mps000485ExtendSingleKey.FREE_CO_PAID_TIME, rdo.patientTypeAlter.FREE_CO_PAID_TIME));
                    SetSingleKey(new KeyValue(Mps000485ExtendSingleKey.ALTER_HEIN_MEDI_ORG_CODE, rdo.patientTypeAlter.HEIN_MEDI_ORG_CODE));
                    SetSingleKey(new KeyValue(Mps000485ExtendSingleKey.ALTER_HEIN_MEDI_ORG_NAME, rdo.patientTypeAlter.HEIN_MEDI_ORG_NAME));
                }
            }
            catch (Exception ex)
            {
