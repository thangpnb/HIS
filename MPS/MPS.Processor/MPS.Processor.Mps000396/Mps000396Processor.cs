...
using MPS.Processor.Mps000396.PDO;
...
namespace MPS.Processor.Mps000396
{
    public class Mps000396Processor : AbstractProcessor
    {
        Mps000396PDO rdo;

        public Mps000396Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000396PDO)rdoBase;
        }
        ...
        private void SetSingleKey()
        {
            try
            {
                if (this.rdo != null)
                {
                    ...
                    {
                        SetSingleKey(new KeyValue(Mps000396ExtendSingleKey.DOB_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo._Treatment.TDL_PATIENT_DOB)));
                        SetSingleKey((new KeyValue(Mps000396ExtendSingleKey.DOB_YEAR_STR, rdo._Treatment.TDL_PATIENT_DOB.ToString().Substring(0, 4))));
                        SetSingleKey((new KeyValue(Mps000396ExtendSingleKey.IN_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo._Treatment.IN_TIME))));
                        SetSingleKey((new KeyValue(Mps000396ExtendSingleKey.CLINICAL_IN_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo._Treatment.CLINICAL_IN_TIME ?? 0))));
                        SetSingleKey(new KeyValue(Mps000396ExtendSingleKey.AGE_STR, AgeUtil.CalculateFullAge(rdo._Treatment.TDL_PATIENT_DOB)));
                    ...
                    if (rdo._SereServExt != null)
                    {
                        ...
                        {
                            SetSingleKey((new KeyValue(Mps000396ExtendSingleKey.BEGIN_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo._SereServExt.BEGIN_TIME.Value))));
                            ...
                            SetSingleKey((new KeyValue(Mps000396ExtendSingleKey.BEGIN_HOUR_STR, hour)));
                        }
