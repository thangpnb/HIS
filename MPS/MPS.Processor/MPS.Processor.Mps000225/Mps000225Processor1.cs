...
<<<<<<< .mine
||||||| .r110322
                    ado.AFTERNOON_STR = itemGroup.FirstOrDefault().AFTERNOON_STR;
                    ado.NOON_STR = itemGroup.FirstOrDefault().NOON_STR;
                    ado.MORNING_STR = itemGroup.FirstOrDefault().MORNING_STR;
                    ado.EVENING_STR = itemGroup.FirstOrDefault().EVENING_STR;
=======
                    ado.AFTERNOON = itemGroup.FirstOrDefault().AFTERNOON;
                    ado.NOON = itemGroup.FirstOrDefault().NOON;
                    ado.MORNING = itemGroup.FirstOrDefault().MORNING;
                    ado.EVENING = itemGroup.FirstOrDefault().EVENING;
>>>>>>> .r110410
                    ...
                    foreach (var item in itemGroup)
                    {
                        for (int j = 0; j < 60; j++)
                        {
                            ...
                            PropertyInfo infoMorning = ps.FirstOrDefault(o => o.Name == string.Format("MORNING_Day{0}", j + 1));
                            if (infoMorning != null)
                            {
                                decimal itemValue = Inventec.Common.TypeConvert.Parse.ToDecimal((infoMorning.GetValue(item) ?? "0").ToString());
                                decimal adoValue = Inventec.Common.TypeConvert.Parse.ToDecimal((infoMorning.GetValue(ado) ?? "0").ToString());
                                if (itemValue > 0)
                                {
                                    infoMorning.SetValue(ado, (adoValue + itemValue).ToString());
                                }
                            }

                            PropertyInfo infoNoon = ps.FirstOrDefault(o => o.Name == string.Format("NOON_Day{0}", j + 1));
                            if (infoNoon != null)
                            {
                                decimal itemValue = Inventec.Common.TypeConvert.Parse.ToDecimal((infoNoon.GetValue(item) ?? "0").ToString());
                                decimal adoValue = Inventec.Common.TypeConvert.Parse.ToDecimal((infoNoon.GetValue(ado) ?? "0").ToString());
                                if (itemValue > 0)
                                {
                                    infoNoon.SetValue(ado, (adoValue + itemValue).ToString());
                                }
                            }

                            PropertyInfo infoAfternoon = ps.FirstOrDefault(o => o.Name == string.Format("AFTERNOON_Day{0}", j + 1));
                            if (infoAfternoon != null)
                            {
                                decimal itemValue = Inventec.Common.TypeConvert.Parse.ToDecimal((infoAfternoon.GetValue(item) ?? "0").ToString());
                                decimal adoValue = Inventec.Common.TypeConvert.Parse.ToDecimal((infoAfternoon.GetValue(ado) ?? "0").ToString());
                                if (itemValue > 0)
                                {
                                    infoAfternoon.SetValue(ado, (adoValue + itemValue).ToString());
                                }
                            }

                            PropertyInfo infoEvening = ps.FirstOrDefault(o => o.Name == string.Format("EVENING_Day{0}", j + 1));
                            if (infoEvening != null)
                            {
                                decimal itemValue = Inventec.Common.TypeConvert.Parse.ToDecimal((infoEvening.GetValue(item) ?? "0").ToString());
                                decimal adoValue = Inventec.Common.TypeConvert.Parse.ToDecimal((infoEvening.GetValue(ado) ?? "0").ToString());
                                if (itemValue > 0)
                                {
                                    infoEvening.SetValue(ado, (adoValue + itemValue).ToString());
                                }
                            }
                        }
                    }
