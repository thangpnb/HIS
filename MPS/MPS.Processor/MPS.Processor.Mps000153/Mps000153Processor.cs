...
using MPS.Processor.Mps000153.PDO;
...
namespace MPS.Processor.Mps000153
{
    public class Mps000153Processor : AbstractProcessor
    {
        Mps000153PDO rdo;
        public Mps000153Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000153PDO)rdoBase;
        }
        ...
        void ProcessSingleKey()
        {
            try
            {
                ...
                {
                    SetSingleKey(new KeyValue(Mps000153ExtendSingleKey.CREATE_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(rdo._ExpeExpMest.CREATE_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000153ExtendSingleKey.CREATE_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo._ExpeExpMest.CREATE_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000153ExtendSingleKey.CREATE_DATE_SEPARATE, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(rdo._ExpeExpMest.CREATE_TIME ?? 0)));

                    SetSingleKey(new KeyValue(Mps000153ExtendSingleKey.EXP_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(rdo._ExpeExpMest.EXP_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000153ExtendSingleKey.EXP_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo._ExpeExpMest.EXP_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000153ExtendSingleKey.EXP_DATE_SEPARATE, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(rdo._ExpeExpMest.EXP_TIME ?? 0)));
                ...
                if (rdo._Medicines != null && rdo._Medicines.Count > 0)
                {
                    ...
                    foreach (var group in Group)
                    {
                        ...
                        {
                            rdo.listAdo.Add(new Mps000153ADO(dic.Value));
                        }
                    }
                }
                ...
                SetSingleKey(new KeyValue(Mps000153ExtendSingleKey.SUM_TOTAL_PRICE, totalPrice));
                string sumText = String.Format("{0:0.####}", Inventec.Common.Number.Convert.NumberToNumberRoundMax4(totalPrice));
                SetSingleKey(new KeyValue(Mps000153ExtendSingleKey.SUM_TOTAL_PRICE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(sumText)));

                rdo.listAdo = rdo.listAdo.OrderBy(o => o.TYPE_ID).ThenByDescending(t => t.NUM_ORDER).ToList();
                if (rdo.listAdo != null && rdo.listAdo.Count > 0)
                {
                    SetSingleKey(new KeyValue(Mps000153ExtendSingleKey.SUM_TOTAL_AMOUNT, rdo.listAdo.Count));
                }
            }
            catch (Exception ex)
            {
