...
using MPS.Processor.Mps000350.PDO;
...
{
    class Mps000350Processor : AbstractProcessor
    {
        Mps000350PDO rdo;
        public Mps000350Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000350PDO)rdoBase;
        }
        ...
        private void SetSingleKey()
        {
            try
            {
                ...
                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.NUM_ORDER, rdo.Transaction.NUM_ORDER));

                if (this.rdo._SaleExpMests != null && this.rdo._SaleExpMests.Count > 0)
                {
                    SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.CREATE_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(this.rdo._SaleExpMests.OrderByDescending(o => o.EXP_MEST_CODE).FirstOrDefault().CREATE_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.CREATE_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(this.rdo._SaleExpMests.OrderByDescending(o => o.EXP_MEST_CODE).FirstOrDefault().CREATE_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.CREATE_DATE_SEPARATE, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(this.rdo._SaleExpMests.OrderByDescending(o => o.EXP_MEST_CODE).FirstOrDefault().CREATE_TIME ?? 0)));

                    var exp = this.rdo._SaleExpMests.FirstOrDefault(o => o.TDL_PATIENT_ID.HasValue);
                    if (exp != null)
                    {
                        SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.PATIENT_NAME, exp.TDL_PATIENT_NAME));
                        AddObjectKeyIntoListkey(exp, false);
                    }
                    else
                    {
                        SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.PATIENT_NAME, this.rdo._SaleExpMests.FirstOrDefault().TDL_PATIENT_NAME));
                        SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.VIR_PATIENT_NAME, this.rdo._SaleExpMests.FirstOrDefault().TDL_PATIENT_NAME));
                        AddObjectKeyIntoListkey(this.rdo._SaleExpMests.FirstOrDefault(), false);
                    }
                    SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.EXP_MEST_CODES, String.Join(",", this.rdo._SaleExpMests.Select(s => s.EXP_MEST_CODE).ToList())));
                ...
                if (this.rdo._Medicines != null && this.rdo._Medicines.Count > 0)
                {
                    ...
                    foreach (var group in Group)
                    {
                        ...
                        {
                            Mps000350ADO ado = new Mps000350ADO(dic.Value);
                ...
                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.EXP_TIME, expTime));
                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.APPROVAL_LOGINNAME, approvalLoginname));
                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.EXP_LOGINNAME, expLoginName));
                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.SUM_TOTAL_PRICE, totalPrice));
                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.SUM_TOTAL_PRICE_NOT_ROUND, totalPriceNotRound));
                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.DISCOUNT, discount));
                string sumText = String.Format("{0:0.####}", Inventec.Common.Number.Convert.NumberToNumberRoundMax4(totalPrice));
                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.SUM_TOTAL_PRICE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(sumText)));
                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.SUM_TOTAL_PRICE_AFTER_DISCOUNT, Inventec.Common.Number.Convert.NumberToString(totalPrice - discount)));
                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.SUM_TOTAL_PRICE_AFTER_DISCOUNT_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(String.Format("{0:0.####}", Inventec.Common.Number.Convert.NumberToNumberRoundMax4(totalPrice - discount)))));

                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.SUM_TOTAL_PRICE_NO_TH, totalPriceNoTh));

                string sumNotRoundText = String.Format("{0:0}", totalPriceNotRound);
                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.SUM_TOTAL_PRICE_NOT_ROUND_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(sumNotRoundText)));
                decimal sumAfterDiscountNotRound = Math.Round((totalPriceNotRound - discount), 0, MidpointRounding.AwayFromZero);
                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.SUM_TOTAL_PRICE_AFTER_DISCOUNT_NOT_ROUND, sumAfterDiscountNotRound));
                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.SUM_TOTAL_PRICE_AFTER_DISCOUNT_NOT_ROUND_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(String.Format("{0:0}", sumAfterDiscountNotRound))));
                ...
                SetSingleKey(new KeyValue(Mps000350ExtendSingleKey.MOBA_IMP_MEST_COUNT, mobaImpMestCount));
        ...
        private void SetBarcodeKey()
        {
            try
            {
                if (rdo.Transaction != null && !String.IsNullOrEmpty(rdo.Transaction.TRANSACTION_CODE))
                {
                    ...
                    dicImage.Add(Mps000350ExtendSingleKey.TRANSACTION_CODE_BAR, barcodeTransactionCode);
                }
                ...
                if (rdo._SaleExpMests != null && rdo._SaleExpMests.Count > 0)
                {
                    ...
                    dicImage.Add(Mps000350ExtendSingleKey.EXP_MEST_CODE_BAR, barcodeExpMestCode);
                }
            }
            catch (Exception ex)
            {
