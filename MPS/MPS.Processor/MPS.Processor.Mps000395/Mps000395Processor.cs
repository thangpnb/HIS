...
using MPS.Processor.Mps000395.PDO;
...
namespace MPS.Processor.Mps000395
{
    public class Mps000395Processor : AbstractProcessor
    {
        Mps000395PDO rdo;

        public Mps000395Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000395PDO)rdoBase;
        }
        ...
        private void SetSingleKey()
        {
            try
            {
                if (this.rdo != null)
                {
                    ...
                    {
                        SetSingleKey(new KeyValue(Mps000395ExtendSingleKey.DOB_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo._Treatment.TDL_PATIENT_DOB)));
                        SetSingleKey((new KeyValue(Mps000395ExtendSingleKey.DOB_YEAR_STR, rdo._Treatment.TDL_PATIENT_DOB.ToString().Substring(0, 4))));
                        SetSingleKey((new KeyValue(Mps000395ExtendSingleKey.IN_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo._Treatment.IN_TIME))));
                        SetSingleKey((new KeyValue(Mps000395ExtendSingleKey.CLINICAL_IN_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo._Treatment.CLINICAL_IN_TIME ?? 0))));
                        SetSingleKey(new KeyValue(Mps000395ExtendSingleKey.AGE_STR, AgeUtil.CalculateFullAge(rdo._Treatment.TDL_PATIENT_DOB)));
                    ...
                    if (rdo._SereServExt != null)
                    {
                        ...
                        {
                            SetSingleKey((new KeyValue(Mps000395ExtendSingleKey.BEGIN_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo._SereServExt.BEGIN_TIME.Value))));
                            ...
                            SetSingleKey((new KeyValue(Mps000395ExtendSingleKey.BEGIN_HOUR_STR, hour)));
                        }
