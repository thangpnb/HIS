...
using MPS.Processor.Mps100007.PDO;
...
namespace MPS.Processor.Mps100007
{
    class Mps100007Processor : AbstractProcessor
    {
        Mps100007PDO rdo;

        public Mps100007Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps100007PDO)rdoBase;
        }
        ...
        public void SetBarcodeKey()
        {
            try
            {
                if (rdo.currentHisTreatment != null && !String.IsNullOrWhiteSpace(rdo.currentHisTreatment.TREATMENT_CODE))
                {
                    ...
                    dicImage.Add(Mps100007ExtendSingleKey.TREATMENT_CODE_BAR, barcodeTreatmentCode);
                }
                ...
                if (rdo.currentHisTreatment != null && !String.IsNullOrWhiteSpace(rdo.currentHisTreatment.TDL_PATIENT_CODE))
                {
                    ...
                    dicImage.Add(Mps100007ExtendSingleKey.PATIENT_CODE_BAR, barcodeTreatmentCode);
                }
                ...
                if (rdo.deposit != null && !String.IsNullOrWhiteSpace(rdo.deposit.TRANSACTION_CODE))
                {
                    ...
                    dicImage.Add(Mps100007ExtendSingleKey.TRANSACTION_CODE_BAR, barcodeTreatmentCode);
                }
            }
            catch (Exception ex)
            {
        ...
        void ProcessSingleKey()
        {
            try
            {
                if (rdo.departmentTrans != null && rdo.departmentTrans.Count > 0)
                {
                    ...
                    SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.OPEN_TIME_SEPARATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(rdo.departmentTrans[0].DEPARTMENT_IN_TIME ?? 0)));
                ...
                //Số ngày điều trị
                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.TOTAL_DAY, rdo.totalDay));
                if (this.rdo.hisServiceReq != null)
                {
                    SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.FIRST_EXAM_ROOM_NAME, this.rdo.hisServiceReq.EXECUTE_ROOM_NAME));
                }

                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.RATIO, this.rdo.ratio));

                if (rdo.departmentName != null)
                {
                    SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.DEPARTMENT_NAME, rdo.departmentName));
                }
                ...
                if (rdo.patyAlterBhytADO != null)
                {
                    if (rdo.patyAlterBhytADO.RIGHT_ROUTE_CODE == MOS.LibraryHein.Bhyt.HeinRightRoute.HeinRightRouteCode.TRUE)
                    {
                        ...
                        {
                            SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME_CC, "X"));
                            SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME, ""));
                            SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.NOT_RIGHT_ROUTE_TYPE_NAME, ""));
                        }
                        ...
                        {
                            SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME_CC, ""));
                            SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME, "X"));
                            SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.NOT_RIGHT_ROUTE_TYPE_NAME, ""));
                        }
                        else
                        {
                    ...
                    else if (rdo.patyAlterBhytADO.RIGHT_ROUTE_CODE == MOS.LibraryHein.Bhyt.HeinRightRoute.HeinRightRouteCode.FALSE)//trai tuyen
                    {
                        SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME_CC, ""));
                        SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME, ""));
                        SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.NOT_RIGHT_ROUTE_TYPE_NAME, "X"));
                    }

                    if (!String.IsNullOrEmpty(rdo.patyAlterBhytADO.HEIN_CARD_NUMBER))
                    {
                        SetSingleKey((new KeyValue(Mps100007ExtendSingleKey.HEIN_CARD_NUMBER_SEPARATE, SetHeinCardNumberDisplayByNumber(rdo.patyAlterBhytADO.HEIN_CARD_NUMBER))));
                        SetSingleKey((new KeyValue(Mps100007ExtendSingleKey.IS_HEIN, "X")));
                        SetSingleKey((new KeyValue(Mps100007ExtendSingleKey.IS_VIENPHI, "")));
                        SetSingleKey((new KeyValue(Mps100007ExtendSingleKey.HEIN_CARD_NUMBER_1, rdo.patyAlterBhytADO.HEIN_CARD_NUMBER.Substring(0, 2))));
                        SetSingleKey((new KeyValue(Mps100007ExtendSingleKey.HEIN_CARD_NUMBER_2, rdo.patyAlterBhytADO.HEIN_CARD_NUMBER.Substring(2, 1))));
                        SetSingleKey((new KeyValue(Mps100007ExtendSingleKey.HEIN_CARD_NUMBER_3, rdo.patyAlterBhytADO.HEIN_CARD_NUMBER.Substring(3, 2))));
                        SetSingleKey((new KeyValue(Mps100007ExtendSingleKey.HEIN_CARD_NUMBER_4, rdo.patyAlterBhytADO.HEIN_CARD_NUMBER.Substring(5, 2))));
                        SetSingleKey((new KeyValue(Mps100007ExtendSingleKey.HEIN_CARD_NUMBER_5, rdo.patyAlterBhytADO.HEIN_CARD_NUMBER.Substring(7, 3))));
                        SetSingleKey((new KeyValue(Mps100007ExtendSingleKey.HEIN_CARD_NUMBER_6, rdo.patyAlterBhytADO.HEIN_CARD_NUMBER.Substring(10, 5))));
                        SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.STR_HEIN_CARD_FROM_TIME, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo.patyAlterBhytADO.HEIN_CARD_FROM_TIME ?? 0)));
                        SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.STR_HEIN_CARD_TO_TIME, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo.patyAlterBhytADO.HEIN_CARD_TO_TIME ?? 0)));
                        SetSingleKey((new KeyValue(Mps100007ExtendSingleKey.HEIN_MEDI_ORG_CODE, rdo.patyAlterBhytADO.HEIN_MEDI_ORG_CODE)));
                        SetSingleKey((new KeyValue(Mps100007ExtendSingleKey.HEIN_MEDI_ORG_NAME, rdo.patyAlterBhytADO.HEIN_MEDI_ORG_NAME)));
                    }
                    else
                    {
                        SetSingleKey((new KeyValue(Mps100007ExtendSingleKey.IS_HEIN, "")));
                        SetSingleKey((new KeyValue(Mps100007ExtendSingleKey.IS_VIENPHI, "X")));
                    }
                }
                ...
                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.VIR_TOTAL_PRICE_HEIN_LIMIT, rdo.thuChenhLech));
                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.VIR_DEPOSIT_AMOUNT, rdo.thuDongChiTra));

                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.TOTAL_PRICE, thanhtien_tong));
                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.TOTAL_PRICE_HEIN, bhytthanhtoan_tong));
                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.TOTAL_PRICE_PATIENT, bnthanhtoan_tong));
                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.TOTAL_PRICE_OTHER, Inventec.Common.Number.Convert.NumberToStringRoundAuto(nguonkhac_tong, 0)));
                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.TOTAL_PRICE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(thanhtien_tong).ToString())));
                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.TOTAL_PRICE_HEIN_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(bhytthanhtoan_tong).ToString())));
                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.TOTAL_PRICE_PATIENT_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(bnthanhtoan_tong).ToString())));
                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.TOTAL_PRICE_OTHER_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(nguonkhac_tong).ToString())));

                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.VIR_TOTAL_PRICE_HEIN_LIMIT_TO_VN, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(rdo.thuChenhLech).ToString())));
                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.VIR_DEPOSIT_AMOUNT_TO_VN, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(rdo.thuDongChiTra).ToString())));
                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.NUM_ORDER_TRANSACTION, this.rdo.deposit.NUM_ORDER));
                ...
                SetSingleKey(new KeyValue(Mps100007ExtendSingleKey.HEIN_CARD_NUMBER, rdo.currentHisTreatment.TDL_HEIN_CARD_NUMBER));
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
