...
using MPS.Processor.Mps000344.PDO;
...
{
    public class Mps000344Processor : AbstractProcessor
    {
        Mps000344PDO rdo;
        List<Mps000344ADO> ListAdo = new List<Mps000344ADO>();
        public Mps000344Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000344PDO)rdoBase;
        }
        ...
        void ProcessSingleKey()
        {
            try
            {
                if (rdo._Transaction != null)
                {
                    ...
                    SetSingleKey(new KeyValue(Mps000344ExtendSingleKey.AMOUNT_TEXT_UPPER_FIRST, amountText));
                    decimal amountAfterExem = rdo._Transaction.AMOUNT - (rdo._Transaction.EXEMPTION ?? 0);
                    SetSingleKey(new KeyValue(Mps000344ExtendSingleKey.AMOUNT_AFTER_EXEMPTION, amountAfterExem));
                    ...
                    SetSingleKey(new KeyValue(Mps000344ExtendSingleKey.AMOUNT_AFTER_EXEMPTION_TEXT, amountAfterExemText));
                    SetSingleKey(new KeyValue(Mps000344ExtendSingleKey.AMOUNT_AFTER_EXEMPTION_TEXT_UPPER_FIRST, amountAfterExemText));
                    decimal ratio = ((rdo._Transaction.EXEMPTION ?? 0) * 100) / rdo._Transaction.AMOUNT;
                    SetSingleKey(new KeyValue(Mps000344ExtendSingleKey.EXEMPTION_RATIO, Inventec.Common.Number.Convert.NumberToNumberRoundMax4(ratio)));
                    ...
                    SetSingleKey(new KeyValue(Mps000344ExtendSingleKey.CT_AMOUNT, Inventec.Common.Number.Convert.NumberToNumberRoundMax4(canthu)));
                    SetSingleKey(new KeyValue(Mps000344ExtendSingleKey.CT_AMOUNT_TEXT_UPPER_FIRST, Inventec.Common.Number.Convert.NumberToStringRoundAuto(canthu, 4)));
                    ...
                    if (this.rdo._Medicines != null && this.rdo._Medicines.Count > 0)
                    {
                        ...
                        foreach (var group in Group)
                        {
                            ...
                            {
                                Mps000344ADO ado = new Mps000344ADO(dic.Value);
                    ...
                    if (this.rdo._BillGoods != null && this.rdo._BillGoods.Count > 0 && ListAdo.Count <= 0)
                    {
                        foreach (var item in this.rdo._BillGoods)
                        {
                            ...
                            Mps000344ADO ado = new Mps000344ADO(item);
                            totalPriceNotRound += ado.SUM_TOTAL_PRICE_ROUND;
                            ListAdo.Add(ado);
                        }
                    }
                    ...
                    SetSingleKey(new KeyValue(Mps000344ExtendSingleKey.SUM_TOTAL_PRICE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(sumText)));
                    SetSingleKey(new KeyValue(Mps000344ExtendSingleKey.SUM_TOTAL_PRICE_ROUND, totalPriceNotRound));
                    string sumNotRoundText = String.Format("{0:0}", totalPriceNotRound);
                    SetSingleKey(new KeyValue(Mps000344ExtendSingleKey.SUM_TOTAL_PRICE_ROUND_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(sumNotRoundText)));
                }
            }
            catch (Exception ex)
            {
