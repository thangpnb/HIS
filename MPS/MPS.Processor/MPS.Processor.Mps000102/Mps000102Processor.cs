...
{
    class Mps000102Processor : AbstractProcessor
    {
        Mps000102PDO rdo;

        private List<SereServPlusADO> ExecuteRoomGroups { get; set; }
        private List<SereServPlusADO> ServiceTypeGroups { get; set; }

        private List<SereServPlusADO> ListSereServTotal = new List<SereServPlusADO>();
        private List<SereServPlusADO> ServiceReqGroupTotals = new List<SereServPlusADO>();
        private List<SereServPlusADO> ServiceTypeGroupTotals = new List<SereServPlusADO>();

        public Mps000102Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000102PDO)rdoBase;
        }
        ...
        public void SetBarcodeKey()
        {
            try
            {
                if (rdo.currentHisTreatment != null && !String.IsNullOrWhiteSpace(rdo.currentHisTreatment.TREATMENT_CODE))
                {
                    ...
                    dicImage.Add(Mps000102ExtendSingleKey.TREATMENT_CODE_BAR, barcodeTreatmentCode);
                }
                ...
                if (rdo.currentHisTreatment != null && !String.IsNullOrWhiteSpace(rdo.currentHisTreatment.TDL_PATIENT_CODE))
                {
                    ...
                    dicImage.Add(Mps000102ExtendSingleKey.PATIENT_CODE_BAR, barcodeTreatmentCode);
                }
                ...
                if (rdo.deposit != null && !String.IsNullOrWhiteSpace(rdo.deposit.TRANSACTION_CODE))
                {
                    ...
                    dicImage.Add(Mps000102ExtendSingleKey.TRANSACTION_CODE_BAR, barcodeTreatmentCode);
                }
            }
            catch (Exception ex)
            {
        ...
        public override bool ProcessData()
        {
            ...
            try
            {
                ...
                if (ExecuteRoomGroups == null)
                {
                    ExecuteRoomGroups = new List<SereServPlusADO>();
                }

                ServiceTypeGroups = ExecuteRoomGroups.GroupBy(o => o.TDL_SERVICE_TYPE_ID).Select(s => s.First()).ToList();
                if (ServiceTypeGroups == null)
                {
                    ServiceTypeGroups = new List<SereServPlusADO>();
                }

                ServiceTypeGroups = ServiceTypeGroups.OrderBy(o => o.SERVICE_TYPE_NUM_ORDER ?? 9999999999).ToList();
                ...
                objectTag.AddRelationship(store, "ServiceTypeGroup", "ExecuteRoomGroups", "TDL_SERVICE_TYPE_ID", "TDL_SERVICE_TYPE_ID");
                objectTag.AddRelationship(store, "ServiceTypeGroup", "Services", "TDL_SERVICE_TYPE_ID", "TDL_SERVICE_TYPE_ID");
                objectTag.AddRelationship(store, "ServiceTypeGroup", "ServiceHightTechs", "TDL_SERVICE_TYPE_ID", "TDL_SERVICE_TYPE_ID");
                objectTag.AddRelationship(store, "ServiceTypeGroup", "ServiceVTTTs", "TDL_SERVICE_TYPE_ID", "TDL_SERVICE_TYPE_ID");

                objectTag.AddRelationship(store, "ExecuteRoomGroups", "Services", "TDL_EXECUTE_ROOM_ID", "TDL_EXECUTE_ROOM_ID");
                objectTag.AddRelationship(store, "ExecuteRoomGroups", "ServiceHightTechs", "TDL_EXECUTE_ROOM_ID", "TDL_EXECUTE_ROOM_ID");
                objectTag.AddRelationship(store, "ExecuteRoomGroups", "ServiceVTTTs", "TDL_EXECUTE_ROOM_ID", "TDL_EXECUTE_ROOM_ID");

                if (ListSereServTotal != null && ListSereServTotal.Count > 0)
                {
                    ...
                    List<SereServPlusADO> SereServTotal = new List<SereServPlusADO>();
                    ...
                    {
                        SereServPlusADO sTypeAdo = new SereServPlusADO(sType.First().First());
                        ...
                        {
                            SereServPlusADO reqAdo = new SereServPlusADO(req.First());
        ...
        void ProcessSingleKey()
        {
            try
            {
                ...
                {
                    SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.OPEN_TIME_SEPARATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(rdo.departmentTrans[0].DEPARTMENT_IN_TIME ?? 0)));
                }

                //Số ngày điều trị
                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.TOTAL_DAY, rdo.totalDay));
                if (this.rdo.hisServiceReq != null)
                {
                    SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.FIRST_EXAM_ROOM_NAME, this.rdo.hisServiceReq.EXECUTE_ROOM_NAME));
                }

                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.RATIO, this.rdo.ratio));

                if (rdo.departmentName != null)
                {
                    SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.DEPARTMENT_NAME, rdo.departmentName));
                }
                ...
                if (rdo.patyAlterBhytADO != null)
                {
                    if (rdo.patyAlterBhytADO.RIGHT_ROUTE_CODE == MOS.LibraryHein.Bhyt.HeinRightRoute.HeinRightRouteCode.TRUE)
                    {
                        ...
                        {
                            SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME_CC, "X"));
                            SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME, ""));
                            SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.NOT_RIGHT_ROUTE_TYPE_NAME, ""));
                        }
                        ...
                        {
                            SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME_CC, ""));
                            SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME, "X"));
                            SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.NOT_RIGHT_ROUTE_TYPE_NAME, ""));
                        }
                        else
                        {
                    ...
                    else if (rdo.patyAlterBhytADO.RIGHT_ROUTE_CODE == MOS.LibraryHein.Bhyt.HeinRightRoute.HeinRightRouteCode.FALSE)//trai tuyen
                    {
                        SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME_CC, ""));
                        SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.RIGHT_ROUTE_TYPE_NAME, ""));
                        SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.NOT_RIGHT_ROUTE_TYPE_NAME, "X"));
                    }

                    if (!String.IsNullOrEmpty(rdo.patyAlterBhytADO.HEIN_CARD_NUMBER))
                    {
                        SetSingleKey((new KeyValue(Mps000102ExtendSingleKey.HEIN_CARD_NUMBER_SEPARATE, SetHeinCardNumberDisplayByNumber(rdo.patyAlterBhytADO.HEIN_CARD_NUMBER))));
                        SetSingleKey((new KeyValue(Mps000102ExtendSingleKey.IS_HEIN, "X")));
                        SetSingleKey((new KeyValue(Mps000102ExtendSingleKey.IS_VIENPHI, "")));
                        SetSingleKey((new KeyValue(Mps000102ExtendSingleKey.HEIN_CARD_NUMBER_1, rdo.patyAlterBhytADO.HEIN_CARD_NUMBER.Substring(0, 2))));
                        SetSingleKey((new KeyValue(Mps000102ExtendSingleKey.HEIN_CARD_NUMBER_2, rdo.patyAlterBhytADO.HEIN_CARD_NUMBER.Substring(2, 1))));
                        SetSingleKey((new KeyValue(Mps000102ExtendSingleKey.HEIN_CARD_NUMBER_3, rdo.patyAlterBhytADO.HEIN_CARD_NUMBER.Substring(3, 2))));
                        SetSingleKey((new KeyValue(Mps000102ExtendSingleKey.HEIN_CARD_NUMBER_4, rdo.patyAlterBhytADO.HEIN_CARD_NUMBER.Substring(5, 2))));
                        SetSingleKey((new KeyValue(Mps000102ExtendSingleKey.HEIN_CARD_NUMBER_5, rdo.patyAlterBhytADO.HEIN_CARD_NUMBER.Substring(7, 3))));
                        SetSingleKey((new KeyValue(Mps000102ExtendSingleKey.HEIN_CARD_NUMBER_6, rdo.patyAlterBhytADO.HEIN_CARD_NUMBER.Substring(10, 5))));
                        SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.STR_HEIN_CARD_FROM_TIME, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo.patyAlterBhytADO.HEIN_CARD_FROM_TIME ?? 0)));
                        SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.STR_HEIN_CARD_TO_TIME, Inventec.Common.DateTime.Convert.TimeNumberToDateString(rdo.patyAlterBhytADO.HEIN_CARD_TO_TIME ?? 0)));
                        SetSingleKey((new KeyValue(Mps000102ExtendSingleKey.HEIN_MEDI_ORG_CODE, rdo.patyAlterBhytADO.HEIN_MEDI_ORG_CODE)));
                        SetSingleKey((new KeyValue(Mps000102ExtendSingleKey.HEIN_MEDI_ORG_NAME, rdo.patyAlterBhytADO.HEIN_MEDI_ORG_NAME)));
                    }
                    else
                    {
                        SetSingleKey((new KeyValue(Mps000102ExtendSingleKey.IS_HEIN, "")));
                        SetSingleKey((new KeyValue(Mps000102ExtendSingleKey.IS_VIENPHI, "X")));
                    }
                }
                ...
                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.VIR_TOTAL_PRICE_HEIN_LIMIT, rdo.thuChenhLech));
                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.VIR_DEPOSIT_AMOUNT, rdo.thuDongChiTra));

                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.TOTAL_PRICE, thanhtien_tong));
                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.TOTAL_PRICE_HEIN, bhytthanhtoan_tong));
                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.TOTAL_PRICE_PATIENT, bnthanhtoan_tong));
                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.TOTAL_PRICE_OTHER, Inventec.Common.Number.Convert.NumberToStringRoundAuto(nguonkhac_tong, 0)));
                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.TOTAL_PRICE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(thanhtien_tong).ToString())));
                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.TOTAL_PRICE_HEIN_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(bhytthanhtoan_tong).ToString())));
                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.TOTAL_PRICE_PATIENT_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(bnthanhtoan_tong).ToString())));
                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.TOTAL_PRICE_OTHER_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(nguonkhac_tong).ToString())));

                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.VIR_TOTAL_PRICE_HEIN_LIMIT_TO_VN, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(rdo.thuChenhLech).ToString())));
                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.VIR_DEPOSIT_AMOUNT_TO_VN, Inventec.Common.String.Convert.CurrencyToVneseString(Math.Round(rdo.thuDongChiTra).ToString())));
                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.NUM_ORDER_TRANSACTION, this.rdo.deposit.NUM_ORDER));
                ...
                SetSingleKey(new KeyValue(Mps000102ExtendSingleKey.HEIN_CARD_NUMBER, rdo.currentHisTreatment.TDL_HEIN_CARD_NUMBER));

                var groups = ListSereServTotal.OrderBy(p => p.TDL_SERVICE_TYPE_ID).GroupBy(o => o.TDL_SERVICE_TYPE_ID).ToList();

                var serviceTypes = HIS.Desktop.LocalStorage.BackendData.BackendDataWorker.Get<HIS_SERVICE_TYPE>().ToList();
                ...
                if (groups != null && groups.Count > 0)
                {
                    foreach (var itemgroup in groups)
                    {
                        ...
                        temp.Add(string.Format("{0}[{1}]", name, total));

                    }
                }

                SetSingleKey(new KeyValue("TRANS_DESCRIPTION", String.Join(",", temp)));

                List<long> packageIds = ListSereServTotal.Select(s => s.PACKAGE_ID ?? 0).Distinct().ToList();
                List<string> packageName = new List<string>();
                if (packageIds != null)
                {
                    var package = HIS.Desktop.LocalStorage.BackendData.BackendDataWorker.Get<HIS_PACKAGE>().Where(o => packageIds.Contains(o.ID)).ToList();
                    if (package != null && package.Count > 0)
                    {
                        packageName.AddRange(package.Select(s => s.PACKAGE_NAME));
                    }
                }

                SetSingleKey(new KeyValue("TOTAL_PACKAGE_NAME", String.Join(";", packageName.OrderBy(o => o))));
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        private void ProcessGroupSereServ()
        {
            try
            {
                ...
                List<SereServGroupPlusADO> grExecuteRoom = new List<SereServGroupPlusADO>();

                if (rdo.sereServNotHiTechs != null && rdo.sereServNotHiTechs.Count > 0)
                {
                    ListSereServTotal.AddRange(from r in rdo.sereServNotHiTechs select new SereServPlusADO(r));
                    var grExecuteRoomNotHiTechs = rdo.sereServNotHiTechs.GroupBy(o => new { o.TDL_EXECUTE_ROOM_ID, o.TDL_SERVICE_TYPE_ID }).ToList();
                    foreach (var item in grExecuteRoomNotHiTechs)
                    {
                        SereServGroupPlusADO itemSServiceReportGroup = item.First();
                        itemSServiceReportGroup.TOTAL_PRICE_SERVICE_GROUP = item.Sum(o => o.VIR_TOTAL_PRICE ?? 0);
                        itemSServiceReportGroup.TOTAL_HEIN_PRICE_SERVICE_GROUP = item.Sum(o => o.VIR_TOTAL_HEIN_PRICE ?? 0);
                        itemSServiceReportGroup.TOTAL_PATIENT_PRICE_SERVICE_GROUP = item.Sum(o => o.VIR_TOTAL_PATIENT_PRICE ?? 0);
                        itemSServiceReportGroup.TDL_SERVICE_TYPE_ID = item.First().TDL_SERVICE_TYPE_ID;
                        grExecuteRoom.Add(itemSServiceReportGroup);
                    }
                }

                if (rdo.sereServHitechs != null && rdo.sereServHitechs.Count > 0)
                {
                    ListSereServTotal.AddRange(from r in rdo.sereServHitechs select new SereServPlusADO(r));
                    var grExecuteRoomHitechs = rdo.sereServHitechs.GroupBy(o => new { o.TDL_EXECUTE_ROOM_ID, o.TDL_SERVICE_TYPE_ID }).ToList();
                    foreach (var item in grExecuteRoomHitechs)
                    {
                        SereServGroupPlusADO itemSServiceReportGroup = item.First();
                        itemSServiceReportGroup.TOTAL_PRICE_SERVICE_GROUP = item.Sum(o => o.VIR_TOTAL_PRICE ?? 0);
                        itemSServiceReportGroup.TOTAL_HEIN_PRICE_SERVICE_GROUP = item.Sum(o => o.VIR_TOTAL_HEIN_PRICE ?? 0);
                ...
                if (rdo.sereServVTTTs != null && rdo.sereServVTTTs.Count > 0)
                {
                    ListSereServTotal.AddRange(from r in rdo.sereServVTTTs select new SereServPlusADO(r));
                    var grExecuteRoomVTTTs = rdo.sereServVTTTs.GroupBy(o => new { o.TDL_EXECUTE_ROOM_ID, o.TDL_SERVICE_TYPE_ID }).ToList();
                    foreach (var item in grExecuteRoomVTTTs)
                    {
                        SereServGroupPlusADO itemSServiceReportGroup = item.First();
                        itemSServiceReportGroup.TOTAL_PRICE_SERVICE_GROUP = item.Sum(o => o.VIR_TOTAL_PRICE ?? 0);
                        itemSServiceReportGroup.TOTAL_HEIN_PRICE_SERVICE_GROUP = item.Sum(o => o.VIR_TOTAL_HEIN_PRICE ?? 0);
                ...
                var executeRooms = grExecuteRoom.GroupBy(o => new { o.TDL_EXECUTE_ROOM_ID, o.TDL_SERVICE_TYPE_ID }).ToList();
                foreach (var item in executeRooms)
                {
                    SereServPlusADO ado = new SereServPlusADO(item.First());
                    ado.TOTAL_PRICE_SERVICE_GROUP = item.Sum(o => o.TOTAL_PRICE_SERVICE_GROUP);
                    ado.TOTAL_HEIN_PRICE_SERVICE_GROUP = item.Sum(o => o.TOTAL_HEIN_PRICE_SERVICE_GROUP);
                    ado.TOTAL_PATIENT_PRICE_SERVICE_GROUP = item.Sum(o => o.TOTAL_PATIENT_PRICE_SERVICE_GROUP);
                    ExecuteRoomGroups.Add(ado);
                }
            }
            catch (Exception ex)
            {
