...
using MPS.Processor.Mps000092.PDO;
...
{
    class Mps000092Processor : AbstractProcessor
    {
        Mps000092PDO rdo;
        public Mps000092Processor(CommonParam param, PrintData printData)
            : base(param, printData)
        {
            rdo = (Mps000092PDO)rdoBase;
        }
        ...
        private void SetSingleKey()
        {
            try
            {
                ...
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.NUM_ORDER, rdo.Transaction.NUM_ORDER));

                if (this.rdo._SaleExpMests != null && this.rdo._SaleExpMests.Count > 0)
                {
                    SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.CREATE_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(this.rdo._SaleExpMests.OrderByDescending(o => o.EXP_MEST_CODE).FirstOrDefault().CREATE_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.CREATE_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(this.rdo._SaleExpMests.OrderByDescending(o => o.EXP_MEST_CODE).FirstOrDefault().CREATE_TIME ?? 0)));
                    SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.CREATE_DATE_SEPARATE, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(this.rdo._SaleExpMests.OrderByDescending(o => o.EXP_MEST_CODE).FirstOrDefault().CREATE_TIME ?? 0)));
                    var saleExpMestProcess = this.rdo._SaleExpMests.OrderByDescending(o => o.EXP_MEST_CODE).FirstOrDefault();
                    SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.IS_NOT_IN_WORKING_TIME, saleExpMestProcess.IS_NOT_IN_WORKING_TIME));

                    //var cashierRooms = HIS.Desktop.LocalStorage.BackendData.BackendDataWorker.Get<V_HIS_CASHIER_ROOM>();
                    var cashierRooms = this.rdo._HisCashierRoom;
                    if (saleExpMestProcess != null && cashierRooms != null && cashierRooms.Count > 0)
                    {
                        List<V_HIS_CASHIER_ROOM> cashierRoom = saleExpMestProcess.CASHIER_ROOM_ID.HasValue ? cashierRooms.Where(o => o.ID == saleExpMestProcess.CASHIER_ROOM_ID).ToList() : null;
                        if (cashierRoom != null && cashierRoom.Count() > 0)
                        {
                            SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.CASHIER_ROOM_CODE, cashierRoom[0].CASHIER_ROOM_CODE));
                            SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.CASHIER_ROOM_NAME, cashierRoom[0].CASHIER_ROOM_NAME));
                        }
                    }
                    ...
                    if (exp != null)
                    {
                        SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.PATIENT_NAME, exp.TDL_PATIENT_NAME));
                        AddObjectKeyIntoListkey(exp, false);
                    }
                    else
                    {
                        SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.PATIENT_NAME, this.rdo._SaleExpMests.FirstOrDefault().TDL_PATIENT_NAME));
                        SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.VIR_PATIENT_NAME, this.rdo._SaleExpMests.FirstOrDefault().TDL_PATIENT_NAME));
                        AddObjectKeyIntoListkey(this.rdo._SaleExpMests.FirstOrDefault(), false);
                    }
                    SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.EXP_MEST_CODES, String.Join(",", this.rdo._SaleExpMests.Select(s => s.EXP_MEST_CODE).ToList())));
                    discount = this.rdo._SaleExpMests.Sum(o => (o.DISCOUNT ?? 0));


                }

                if (this.rdo._Treatment != null)
                {
                    SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.HEIN_CARD_FROM_TIME_STR, this.rdo._Treatment.TDL_HEIN_CARD_FROM_TIME));
                    SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.HEIN_CARD_TO_TIME_STR, this.rdo._Treatment.TDL_HEIN_CARD_TO_TIME));
                    SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.ICD_CODE, this.rdo._Treatment.ICD_CODE));
                    SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.ICD_NAME, this.rdo._Treatment.ICD_NAME));
                    SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.ICD_SUB_CODE, this.rdo._Treatment.ICD_SUB_CODE));
                    SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.ICD_TEXT, this.rdo._Treatment.ICD_TEXT));
                }
                if (this.rdo._Medicines != null && this.rdo._Medicines.Count > 0)
                {
                    ...
                    foreach (var group in Group)
                    {
                        ...
                        {
                            Mps000092ADO ado = new Mps000092ADO(dic.Value);
                ...
                if (this.rdo._BillGoods != null && this.rdo._BillGoods.Count > 0)
                {
                    foreach (var item in this.rdo._BillGoods)
                    {
                        ...
                        Mps000092ADO ado = new Mps000092ADO(item);
                        totalPriceNotRound += ado.SUM_PRICE_NOT_ROUNT;
                        rdo.listAdo.Add(ado);
                    }
                }
                ...
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.EXP_TIME, expTime));
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.APPROVAL_LOGINNAME, approvalLoginname));
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.EXP_LOGINNAME, expLoginName));
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.SUM_TOTAL_PRICE, totalPrice));
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.SUM_TOTAL_PRICE_NOT_ROUND, totalPriceNotRound));
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.DISCOUNT, discount));
                string sumText = String.Format("{0:0.####}", Inventec.Common.Number.Convert.NumberToNumberRoundMax4(totalPrice));
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.SUM_TOTAL_PRICE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(sumText)));
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.SUM_TOTAL_PRICE_AFTER_DISCOUNT, Inventec.Common.Number.Convert.NumberToString(totalPrice - discount)));
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.SUM_TOTAL_PRICE_AFTER_DISCOUNT_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(String.Format("{0:0.####}", Inventec.Common.Number.Convert.NumberToNumberRoundMax4(totalPrice - discount)))));

                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.SUM_TOTAL_PRICE_NO_TH, totalPriceNoTh));

                string sumNotRoundText = String.Format("{0:0}", totalPriceNotRound);
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.SUM_TOTAL_PRICE_NOT_ROUND_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(sumNotRoundText)));
                decimal sumAfterDiscountNotRound = Math.Round((totalPriceNotRound - discount), 0, MidpointRounding.AwayFromZero);
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.SUM_TOTAL_PRICE_AFTER_DISCOUNT_NOT_ROUND, sumAfterDiscountNotRound));
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.SUM_TOTAL_PRICE_AFTER_DISCOUNT_NOT_ROUND_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(String.Format("{0:0}", sumAfterDiscountNotRound))));
                ...
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.MOBA_IMP_MEST_COUNT, mobaImpMestCount));
                string transNumOrder = "";
                if (this.rdo._Transactions != null && rdo._Transactions.Count > 0)
                {
                    transNumOrder = string.Join(", ", rdo._Transactions.Select(o => o.NUM_ORDER).ToList());
                }
                SetSingleKey(new KeyValue(Mps000092ExtendSingleKey.TRANSACTION_NUM_ORDER, transNumOrder));
        ...
        private void SetBarcodeKey()
        {
            try
            {
                if (rdo.Transaction != null && !String.IsNullOrEmpty(rdo.Transaction.TRANSACTION_CODE))
                {
                    ...
                    dicImage.Add(Mps000092ExtendSingleKey.TRANSACTION_CODE_BAR, barcodeTransactionCode);
                }
                ...
                if (rdo._SaleExpMests != null && rdo._SaleExpMests.Count > 0)
                {
                    ...
                    dicImage.Add(Mps000092ExtendSingleKey.EXP_MEST_CODE_BAR, barcodeExpMestCode);
                }
            }
            catch (Exception ex)
            {
        ...
        public override string ProcessUniqueCodeData()
        {
            ...
            try
            {
                ...
                if (rdo != null)
                {
                    ...
                    {
                        Inventec.Common.Logging.LogSystem.Info("Transaction not null");
                        var lstExpMestCode = rdo._SaleExpMests.Where(p => p.TDL_TREATMENT_CODE == rdo.Transaction.TDL_TREATMENT_CODE).ToList();
                        if (lstExpMestCode != null && lstExpMestCode.Count > 0)
                        {
                            var expMestCode = lstExpMestCode.FirstOrDefault();
                            if (rdo.Transaction.NUM_ORDER == 0)
                            {
                                result = String.Format("{0} EXP_MEST_CODE:{1} TOTAL_PRICE:{2} ACCOUNT_BOOK_CODE:{3} TRANSACTION_CODE:{4} NUM_ORDER:{5}", this.printTypeCode, expMestCode.EXP_MEST_CODE, expMestCode.TDL_TOTAL_PRICE, rdo.Transaction.ACCOUNT_BOOK_CODE, rdo.Transaction.TRANSACTION_CODE, "");
                            }
                            else
                            {
                                result = String.Format("{0} EXP_MEST_CODE:{1} TOTAL_PRICE:{2} ACCOUNT_BOOK_CODE:{3} TRANSACTION_CODE:{4} NUM_ORDER:{5}", this.printTypeCode, expMestCode.EXP_MEST_CODE, expMestCode.TDL_TOTAL_PRICE, rdo.Transaction.ACCOUNT_BOOK_CODE, rdo.Transaction.TRANSACTION_CODE, rdo.Transaction.NUM_ORDER);
                            }
                        }
                        else
                        {
                            ...
                            result = String.Format("{0} EXP_MEST_CODE:{1} TOTAL_PRICE:{2} ACCOUNT_BOOK_CODE:{3} TRANSACTION_CODE:{4} NUM_ORDER:{5}", this.printTypeCode, "", "", rdo.Transaction.ACCOUNT_BOOK_CODE, rdo.Transaction.TRANSACTION_CODE, rdo.Transaction.NUM_ORDER);
                        }
                    }
                    else
                    {
                        Inventec.Common.Logging.LogSystem.Info("Transaction is null");
                        var lstExpMestCode = rdo._SaleExpMests.ToList();
                        if (lstExpMestCode != null && lstExpMestCode.Count > 0)
                        {
                            var expMestCode = lstExpMestCode.FirstOrDefault();
                            result = String.Format("{0} EXP_MEST_CODE:{1} TOTAL_PRICE:{2} ACCOUNT_BOOK_CODE:{3} TRANSACTION_CODE:{4} NUM_ORDER:{5}", this.printTypeCode, expMestCode.EXP_MEST_CODE, expMestCode.TDL_TOTAL_PRICE, "", "", "");
                        }
                        else
                        {
                            result = String.Format("{0} EXP_MEST_CODE:{1} TOTAL_PRICE:{2} ACCOUNT_BOOK_CODE:{3} TRANSACTION_CODE:{4} NUM_ORDER:{5}", this.printTypeCode, "", "", "", "", "");
                        }
                    }
