...
namespace MPS.Core.Mps000086
{
    public class Mps000086RDO : RDOBase
    {
        ...
        internal List<Mps000086ADO> listAdo = new List<Mps000086ADO>();

        public Mps000086RDO(V_HIS_CHMS_EXP_MEST chmsExpMest, List<V_HIS_EXP_MEST_MEDICINE> listMedicine, List<V_HIS_EXP_MEST_MATERIAL> listMaterial)
        {
        ...
        public Mps000086RDO(V_HIS_CHMS_EXP_MEST chmsExpMest, List<V_HIS_EXP_MEST_MEDICINE> listMedicine, List<V_HIS_EXP_MEST_MATERIAL> listMaterial, long _expMesttSttId__Draft, long _expMesttSttId__Request, long _expMesttSttId__Reject, long _expMesttSttId__Approval, long _expMesttSttId__Export)
        {
            this._ChmsExpMest = chmsExpMest;
            this._Materials = listMaterial;
            this._Medicines = listMedicine;
        ...
        internal override void SetSingleKey()
        {
            try
            {
                ...
                {
                    keyValues.Add(new KeyValue(Mps000086ExtendSingleKey.CREATE_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(this._ChmsExpMest.CREATE_TIME ?? 0)));
                    keyValues.Add(new KeyValue(Mps000086ExtendSingleKey.CREATE_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(this._ChmsExpMest.CREATE_TIME ?? 0)));
                    keyValues.Add(new KeyValue(Mps000086ExtendSingleKey.CREATE_DATE_SEPARATE, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(this._ChmsExpMest.CREATE_TIME ?? 0)));
                ...
                if (this._Medicines != null && this._Medicines.Count > 0)
                {
                    ...
                    foreach (var group in Group)
                    {
                        ...
                        {
                            listAdo.Add(new Mps000086ADO(dic.Value));
                        }
                    }
                }
                ...
                keyValues.Add(new KeyValue(Mps000086ExtendSingleKey.SUM_TOTAL_PRICE, totalPrice));
                string sumText = String.Format("0:0.####", Inventec.Common.Number.Convert.NumberToNumberRoundMax4(totalPrice));
                keyValues.Add(new KeyValue(Mps000086ExtendSingleKey.SUM_TOTAL_PRICE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(sumText)));
