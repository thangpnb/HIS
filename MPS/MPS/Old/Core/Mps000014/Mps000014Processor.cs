...
{
    class Mps000014Processor : ProcessorBase, IProcessorPrint
    {
        Mps000014RDO rdo;
        ...
        internal Mps000014Processor(SAR_PRINT_TYPE config, string fileName, object data, MPS.Printer.PreviewType previewType, string printerName)
            : base(config, (RDOBase)data, previewType, printerName)
        {
            rdo = (Mps000014RDO)data;
        ...
        protected bool ProcessData()
        {
            ...
            try
            {
                ...
                if (rdo.ExesereServs!= null)
                {
                    objectTag.AddObjectData(store, "ServiceTest", rdo.ExesereServs);
                }
                else if (rdo.lstSereServTeinPrint!=null)
                {
                    objectTag.AddObjectData(store, "ServiceTest", rdo.lstSereServTeinPrint);
                }
                result = true;
            }
            catch (Exception ex)
