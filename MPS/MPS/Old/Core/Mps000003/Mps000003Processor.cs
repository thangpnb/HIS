...
{
    class Mps000003Processor : ProcessorBase, IProcessorPrint
    {
        Mps000003RDO rdo;
        ...
        internal Mps000003Processor(SAR_PRINT_TYPE config, string fileName, object data, MPS.Printer.PreviewType previewType, string printerName)
            : base(config, (RDOBase)data, previewType, printerName)
        {
            rdo = (Mps000003RDO)data;
        ...
        bool IProcessorPrint.Run()
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    string[] pathArr = this.fileName.Split('\\');
                    string[] fileArr = pathArr.Last().Split('.');
                    string fileNameOnly = fileArr.Last().ToString();
                    string templateFile = Path.GetTempPath() + (String.IsNullOrEmpty(fileNameOnly) ? "Mps" : fileNameOnly + "__" + DateTime.Now.ToString("yyyyMMddHHmmssfff")) + "__Temp__" + ".xlsx";

                    if (store.OutFile(templateFile))
                    {
                        //Print preview
                        result = PrintPreview(templateFile, this.fileName);
                    }
                }
            }
            catch (Exception ex)
        ...
        internal void SetBarcodeKey()
        {
            try
            {
                ...
                //dicImage.Add(Mps000003ExtendSingleKey.BARCODE_PATIENT_CODE_STR, barcodePatientCode);
                ...
                //dicImage.Add(Mps000003ExtendSingleKey.BARCODE_TREATMENT_CODE_STR, barcodeTreatment);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
