...
{
    class Mps000031Processor : ProcessorBase, IProcessorPrint
    {
        Mps000031RDO rdo;
        ...
        internal Mps000031Processor(SAR_PRINT_TYPE config, string fileName, object data, MPS.Printer.PreviewType previewType, string printerName)
            : base(config, (RDOBase)data, previewType, printerName)
        {
            rdo = (Mps000031RDO)data;
        ...
        internal void SetBarcodeKey()
        {
            try
            {
                ...
                if (rdo.currentHisTreatment != null && !String.IsNullOrEmpty(rdo.currentHisTreatment.TREATMENT_CODE))
                {
                    ...
                    dicImage.Add(Mps000031ExtendSingleKey.BARCODE_TREATMENT_CODE_STR, barcodeTreatment);
                }
            }
            catch (Exception ex)
            {
