...
{
    class Mps000096Processor : ProcessorBase, IProcessorPrint
    {
        Mps000096RDO rdo;
        ...
        internal Mps000096Processor(SAR_PRINT_TYPE config, string fileName, object data, MPS.Printer.PreviewType previewType, string printerName)
            : base(config, (RDOBase)data, previewType, printerName)
        {
            rdo = (Mps000096RDO)data;
        ...
        protected bool ProcessData()
        {
            ...
            try
            {
                ...
                if (rdo.lstResult != null)
                {
                    objectTag.AddObjectData(store, "ServiceTest", rdo.lstResult);
                }
                result = true;
            }
            catch (Exception ex)
