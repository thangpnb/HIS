...
{
    class Mps000101Processor : ProcessorBase, IProcessorPrint
    {
        Mps000101RDO rdo;
        ...
        internal Mps000101Processor(SAR_PRINT_TYPE config, string fileName, object data, MPS.Printer.PreviewType previewType, string printerName)
            : base(config, (RDOBase)data, previewType, printerName)
        {
            rdo = (Mps000101RDO)data;
        ...
        void ProcessListADO()
        {
            try
            {
                ...
                if (impMestManuMedicineTemps != null)
                {
                    foreach (var item in impMestManuMedicineTemps)
                    {
