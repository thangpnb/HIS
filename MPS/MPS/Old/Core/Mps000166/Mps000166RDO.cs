...
namespace MPS.Core.Mps000166
{
    public class Mps000166RDO : RDOBase
    {
        V_HIS_EXPE_EXP_MEST _ExpeExpMest = null;
        ...
        internal List<Mps000166ADO> listAdo = new List<Mps000166ADO>();

        public Mps000166RDO(V_HIS_EXPE_EXP_MEST expeExpMest, List<V_HIS_EXP_MEST_MEDICINE> listMedicine, List<V_HIS_EXP_MEST_MATERIAL> listMaterial)
        {
            this._ExpeExpMest = expeExpMest;
            this._Materials = listMaterial;
            this._Medicines = listMedicine;
        }
        public Mps000166RDO(V_HIS_EXPE_EXP_MEST expeExpMest, List<V_HIS_EXP_MEST_MEDICINE> listMedicine, List<V_HIS_EXP_MEST_MATERIAL> listMaterial, long _expMesttSttId__Draft, long _expMesttSttId__Request, long _expMesttSttId__Reject, long _expMesttSttId__Approval, long _expMesttSttId__Export)
        {
            this._ExpeExpMest = expeExpMest;
            this._Materials = listMaterial;
            this._Medicines = listMedicine;
        ...
        internal override void SetSingleKey()
        {
            try
            {
                ...
                if (this._ExpeExpMest != null)
                {
                    keyValues.Add(new KeyValue(Mps000166ExtendSingleKey.CREATE_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(this._ExpeExpMest.CREATE_TIME ?? 0)));
                    keyValues.Add(new KeyValue(Mps000166ExtendSingleKey.CREATE_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(this._ExpeExpMest.CREATE_TIME ?? 0)));
                    keyValues.Add(new KeyValue(Mps000166ExtendSingleKey.CREATE_DATE_SEPARATE, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(this._ExpeExpMest.CREATE_TIME ?? 0)));

                    keyValues.Add(new KeyValue(Mps000166ExtendSingleKey.EXP_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(this._ExpeExpMest.EXP_TIME ?? 0)));
                    keyValues.Add(new KeyValue(Mps000166ExtendSingleKey.EXP_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(this._ExpeExpMest.EXP_TIME ?? 0)));
                    keyValues.Add(new KeyValue(Mps000166ExtendSingleKey.EXP_DATE_SEPARATE, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(this._ExpeExpMest.EXP_TIME ?? 0)));
                    GlobalQuery.AddObjectKeyIntoListkey(this._ExpeExpMest, keyValues, false);
                }

                if (this._Medicines != null && this._Medicines.Count > 0)
                {
                    this._Medicines = this._Medicines.OrderBy(p => p.ID).ToList();
                    if (_ExpeExpMest.EXP_MEST_STT_ID == expMesttSttId__Approval || _ExpeExpMest.EXP_MEST_STT_ID == expMesttSttId__Export)
                    {
                        this._Medicines = this._Medicines.Where(o => o.IN_EXECUTE == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_MEDICINE.IN_EXECUTE__TRUE).ToList();
                    }
                    else if (_ExpeExpMest.EXP_MEST_STT_ID == expMesttSttId__Request || _ExpeExpMest.EXP_MEST_STT_ID == expMesttSttId__Reject || _ExpeExpMest.EXP_MEST_STT_ID == expMesttSttId__Draft)
                    {
                    ...
                    foreach (var group in Group)
                    {
                        ...
                        {
                            listAdo.Add(new Mps000166ADO(dic.Value));
                        }
                    }
                }
                ...
                keyValues.Add(new KeyValue(Mps000166ExtendSingleKey.SUM_TOTAL_PRICE, totalPrice));
                string sumText = String.Format("{0:0.####}", Inventec.Common.Number.Convert.NumberToNumberRoundMax4(totalPrice));
                keyValues.Add(new KeyValue(Mps000166ExtendSingleKey.SUM_TOTAL_PRICE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(sumText)));

                listAdo = listAdo.OrderBy(o => o.TYPE_ID).ThenByDescending(t => t.NUM_ORDER).ToList();
                if (listAdo != null && listAdo.Count > 0)
                {
                    keyValues.Add(new KeyValue(Mps000166ExtendSingleKey.SUM_TOTAL_AMOUNT, listAdo.Count));
                }
            }
            catch (Exception ex)
            {
