...
{
    class Mps000036Processor : ProcessorBase, IProcessorPrint
    {
        Mps000036RDO rdo;
        ...
        internal Mps000036Processor(SAR_PRINT_TYPE config, string fileName, object data, MPS.Printer.PreviewType previewType, string printerName)
            : base(config, (RDOBase)data, previewType, printerName)
        {
            rdo = (Mps000036RDO)data;
            ...
            try
            {
                if (rdo.ServiceReqPrint != null)
                {
                    if (!String.IsNullOrEmpty(rdo.ServiceReqPrint.TREATMENT_CODE))
                    {
                        ...
                        dicImage.Add(Mps000036ExtendSingleKey.TREATMENT_CODE_BAR, barcodeTreatmentCode);
                    }
                    if (!String.IsNullOrEmpty(rdo.ServiceReqPrint.SERVICE_REQ_CODE))
                    {
                        Inventec.Common.BarcodeLib.Barcode barcodeServiceReqtCode = new Inventec.Common.BarcodeLib.Barcode(rdo.ServiceReqPrint.SERVICE_REQ_CODE);
                        barcodeServiceReqtCode.Alignment = Inventec.Common.BarcodeLib.AlignmentPositions.CENTER;
                        barcodeServiceReqtCode.IncludeLabel = false;
                        barcodeServiceReqtCode.Width = 120;
                        barcodeServiceReqtCode.Height = 40;
                        barcodeServiceReqtCode.RotateFlipType = RotateFlipType.Rotate180FlipXY;
                        barcodeServiceReqtCode.LabelPosition = Inventec.Common.BarcodeLib.LabelPositions.BOTTOMCENTER;
                        barcodeServiceReqtCode.EncodedType = Inventec.Common.BarcodeLib.TYPE.CODE128;
                        barcodeServiceReqtCode.IncludeLabel = true;

                        dicImage.Add(Mps000036ExtendSingleKey.SERVICE_REQ_CODE_BAR, barcodeServiceReqtCode);
                    }
                }

                if (rdo.departmentTran!= null&&!String.IsNullOrEmpty(rdo.departmentTran.TREATMENT_CODE))
                {
