...
namespace MPS.Core.Mps000090
{
    public class Mps000090RDO : RDOBase
    {
        V_HIS_CHMS_EXP_MEST _ChmsExpMest;
        List<V_HIS_EXP_MEST_MEDICINE> _Medicines = null;
        List<V_HIS_EXP_MEST_MATERIAL> _ListMaterial = null;
        ...
        internal List<Mps000090ADO> listAdo = new List<Mps000090ADO>();

        public Mps000090RDO(V_HIS_CHMS_EXP_MEST chmsExpMest, List<V_HIS_EXP_MEST_MEDICINE> listMedicine, List<V_HIS_EXP_MEST_MATERIAL> listMaterial)
        {
            this._ChmsExpMest = chmsExpMest;
            this._ListMaterial = listMaterial;
            this._Medicines = listMedicine;
        }
        public Mps000090RDO(V_HIS_CHMS_EXP_MEST chmsExpMest, List<V_HIS_EXP_MEST_MEDICINE> listMedicine, List<V_HIS_EXP_MEST_MATERIAL> listMaterial, long _expMesttSttId__Draft, long _expMesttSttId__Request, long _expMesttSttId__Reject, long _expMesttSttId__Approval, long _expMesttSttId__Export)
        {
            this._ChmsExpMest = chmsExpMest;
            this._ListMaterial = listMaterial;
            this._Medicines = listMedicine;
        ...
        internal override void SetSingleKey()
        {
            try
            {
                ...
                {
                    keyValues.Add(new KeyValue(Mps000090ExtendSingleKey.CREATE_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(this._ChmsExpMest.CREATE_TIME ?? 0)));
                    keyValues.Add(new KeyValue(Mps000090ExtendSingleKey.CREATE_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(this._ChmsExpMest.CREATE_TIME ?? 0)));
                    keyValues.Add(new KeyValue(Mps000090ExtendSingleKey.CREATE_DATE_SEPARATE, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(this._ChmsExpMest.CREATE_TIME ?? 0)));
                ...
                if (this._Medicines != null && this._Medicines.Count > 0)
                {
                    ...
                    foreach (var group in Group)
                    {
                        ...
                        {
                            listAdo.Add(new Mps000090ADO(dic.Value));
                        }
                    }
                }

                if (this._ListMaterial != null && this._ListMaterial.Count > 0)
                {
                    if (_ChmsExpMest.EXP_MEST_STT_ID == expMesttSttId__Approval || _ChmsExpMest.EXP_MEST_STT_ID == expMesttSttId__Export)
                    {
                        this._ListMaterial = this._ListMaterial.Where(o => o.IN_EXECUTE == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_MATERIAL.IN_EXECUTE__TRUE).ToList();
                    }
                    else if (_ChmsExpMest.EXP_MEST_STT_ID == expMesttSttId__Request || _ChmsExpMest.EXP_MEST_STT_ID == expMesttSttId__Reject || _ChmsExpMest.EXP_MEST_STT_ID == expMesttSttId__Draft)
                    {
                        this._ListMaterial = this._ListMaterial.Where(o => o.IN_REQUEST == IMSys.DbConfig.HIS_RS.HIS_EXP_MEST_MATERIAL.IN_REQUEST__TRUE).ToList();
                    }
                    var Group = this._ListMaterial.GroupBy(g => new { g.MATERIAL_TYPE_ID, g.PACKAGE_NUMBER, g.SUPPLIER_ID }).ToList();
                    foreach (var group in Group)
                    {
                ...
                keyValues.Add(new KeyValue(Mps000090ExtendSingleKey.SUM_TOTAL_PRICE, totalPrice));
                string sumText = String.Format("0:0.####", Inventec.Common.Number.Convert.NumberToNumberRoundMax4(totalPrice));
                keyValues.Add(new KeyValue(Mps000090ExtendSingleKey.SUM_TOTAL_PRICE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(sumText)));
