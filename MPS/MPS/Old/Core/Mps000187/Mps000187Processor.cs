...
{
    class Mps000187Processor : ProcessorBase, IProcessorPrint
    {
        Mps000187RDO rdo;
        ...
        internal Mps000187Processor(SAR_PRINT_TYPE config, string fileName, object data, MPS.Printer.PreviewType previewType, string printerName)
            : base(config, (RDOBase)data, previewType, printerName)
        {
            rdo = (Mps000187RDO)data;
    ...
    && (rdo.sereServServiceADOs == null || rdo.sereServServiceADOs.Count == 0))
        ...
        internal void SetBarcodeKey()
        {
            try
            {
                ...
                dicImage.Add(Mps000187ExtendSingleKey.TREATMENT_CODE_BAR, barcodeTreatment);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
