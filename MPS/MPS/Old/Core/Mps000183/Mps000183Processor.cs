...
{
    class Mps000183Processor : ProcessorBase, IProcessorPrint
    {
        Mps000183RDO rdo;
        ...
        internal Mps000183Processor(SAR_PRINT_TYPE config, string fileName, object data, MPS.Printer.PreviewType previewType, string printerName)
            : base(config, (RDOBase)data, previewType, printerName)
        {
            rdo = (Mps000183RDO)data;
    ...
    && (rdo.sereServServiceADOs == null || rdo.sereServServiceADOs.Count == 0))
        ...
        internal void SetBarcodeKey()
        {
            try
            {
                ...
                dicImage.Add(Mps000183ExtendSingleKey.TREATMENT_CODE_BAR, barcodeTreatment);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
