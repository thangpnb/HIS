...
{
    public class Mps000168RDO : RDOBase
    {
        V_HIS_LOST_EXP_MEST _ExpeExpMest = null;
        ...
        internal List<Mps000168ADO> listAdo = new List<Mps000168ADO>();

        public Mps000168RDO(V_HIS_LOST_EXP_MEST expeExpMest, List<V_HIS_EXP_MEST_MEDICINE> listMedicine, List<V_HIS_EXP_MEST_MATERIAL> listMaterial)
        {
        ...
        public Mps000168RDO(V_HIS_LOST_EXP_MEST expeExpMest, List<V_HIS_EXP_MEST_MEDICINE> listMedicine, List<V_HIS_EXP_MEST_MATERIAL> listMaterial, long _expMesttSttId__Draft, long _expMesttSttId__Request, long _expMesttSttId__Reject, long _expMesttSttId__Approval, long _expMesttSttId__Export)
        {
            this._ExpeExpMest = expeExpMest;
            this._Materials = listMaterial;
            this._Medicines = listMedicine;
        ...
        internal override void SetSingleKey()
        {
            try
            {
                ...
                {
                    keyValues.Add(new KeyValue(Mps000168ExtendSingleKey.CREATE_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(this._ExpeExpMest.CREATE_TIME ?? 0)));
                    keyValues.Add(new KeyValue(Mps000168ExtendSingleKey.CREATE_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(this._ExpeExpMest.CREATE_TIME ?? 0)));
                    keyValues.Add(new KeyValue(Mps000168ExtendSingleKey.CREATE_DATE_SEPARATE, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(this._ExpeExpMest.CREATE_TIME ?? 0)));

                    keyValues.Add(new KeyValue(Mps000168ExtendSingleKey.EXP_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(this._ExpeExpMest.EXP_TIME ?? 0)));
                    keyValues.Add(new KeyValue(Mps000168ExtendSingleKey.EXP_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(this._ExpeExpMest.EXP_TIME ?? 0)));
                    keyValues.Add(new KeyValue(Mps000168ExtendSingleKey.EXP_DATE_SEPARATE, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(this._ExpeExpMest.EXP_TIME ?? 0)));
                ...
                if (this._Medicines != null && this._Medicines.Count > 0)
                {
                    ...
                    foreach (var group in Group)
                    {
                        ...
                        {
                            listAdo.Add(new Mps000168ADO(dic.Value));
                        }
                    }
                }
                ...
                keyValues.Add(new KeyValue(Mps000168ExtendSingleKey.SUM_TOTAL_PRICE, totalPrice));
                string sumText = String.Format("{0:0.####}", Inventec.Common.Number.Convert.NumberToNumberRoundMax4(totalPrice));
                keyValues.Add(new KeyValue(Mps000168ExtendSingleKey.SUM_TOTAL_PRICE_TEXT, Inventec.Common.String.Convert.CurrencyToVneseString(sumText)));

                listAdo = listAdo.OrderBy(o => o.TYPE_ID).ThenByDescending(t => t.NUM_ORDER).ToList();
                if (listAdo != null && listAdo.Count > 0)
                {
                    keyValues.Add(new KeyValue(Mps000168ExtendSingleKey.SUM_TOTAL_AMOUNT, listAdo.Count));
                }
            }
            catch (Exception ex)
            {
