...
namespace MPS.Core.Mps000056
{
    /// <summary>
    /// .
    /// </summary>
    public class Mps000056RDO : RDOBase
    {
        ...
        internal override void SetSingleKey()
        {
            try
            {
                if (expMestMediReact!=null )
                {
                    var react = expMestMediReact.FirstOrDefault();
                    GlobalQuery.AddObjectKeyIntoListkey<ExeMediReactADO>(react, keyValues, false);
                }
                if (expMestMediReact!=null)
                {
                    var react = expMestMediReact.FirstOrDefault();
                ...
                keyValues.Add(new KeyValue(Mps000056ExtendSingleKey.BED_ROOM_NAME, bedRoomName));
