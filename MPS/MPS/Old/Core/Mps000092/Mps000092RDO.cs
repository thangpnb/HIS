...
namespace MPS.Core.Mps000092
{
    public class Mps000092RDO : RDOBase
    {
        ...
        public Mps000092RDO(V_HIS_SALE_EXP_MEST saleExpMest, List<V_HIS_EXP_MEST_MEDICINE> listMedicine, List<V_HIS_EXP_MEST_MATERIAL> listMaterial)
        {
        ...
        public Mps000092RDO(V_HIS_SALE_EXP_MEST saleExpMest, List<V_HIS_EXP_MEST_MEDICINE> listMedicine, List<V_HIS_EXP_MEST_MATERIAL> listMaterial, long _expMesttSttId__Draft, long _expMesttSttId__Request, long _expMesttSttId__Reject, long _expMesttSttId__Approval, long _expMesttSttId__Export)
        {
            this._SaleExpMest = saleExpMest;
            this._Materials = listMaterial;
            this._Medicines = listMedicine;
        ...
        internal override void SetSingleKey()
        {
            try
            {
                ...
                {
                    keyValues.Add(new KeyValue(Mps000092ExtendSingleKey.CREATE_TIME_STR, Inventec.Common.DateTime.Convert.TimeNumberToTimeString(this._SaleExpMest.CREATE_TIME ?? 0)));
                    keyValues.Add(new KeyValue(Mps000092ExtendSingleKey.CREATE_DATE_STR, Inventec.Common.DateTime.Convert.TimeNumberToDateString(this._SaleExpMest.CREATE_TIME ?? 0)));
                    keyValues.Add(new KeyValue(Mps000092ExtendSingleKey.CREATE_DATE_SEPARATE, Inventec.Common.DateTime.Convert.TimeNumberToDateStringSeparateString(this._SaleExpMest.CREATE_TIME ?? 0)));
                    if (this._SaleExpMest.PATIENT_ID.HasValue)
                    {
                        keyValues.Add(new KeyValue(Mps000092ExtendSingleKey.PATIENT_NAME, this._SaleExpMest.VIR_PATIENT_NAME));
                    }
                    else
                    {
                        keyValues.Add(new KeyValue(Mps000092ExtendSingleKey.PATIENT_NAME, this._SaleExpMest.CLIENT_NAME));
                        keyValues.Add(new KeyValue(Mps000092ExtendSingleKey.VIR_PATIENT_NAME, this._SaleExpMest.CLIENT_NAME));
                    }
                    GlobalQuery.AddObjectKeyIntoListkey(this._SaleExpMest, keyValues, false);
                    discount = this._SaleExpMest.DISCOUNT ?? 0;
                }
