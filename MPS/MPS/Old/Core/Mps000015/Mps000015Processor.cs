...
{
    class Mps000015Processor : ProcessorBase, IProcessorPrint
    {
        Mps000015RDO rdo;
        ...
        internal Mps000015Processor(SAR_PRINT_TYPE config, string fileName, object data, MPS.Printer.PreviewType previewType, string printerName)
            : base(config, (RDOBase)data, previewType, printerName)
        {
            rdo = (Mps000015RDO)data;
        ...
        protected bool ProcessData()
        {
            ...
            try
            {
                ...
                if (rdo.lstSereServ != null)
                {
                    objectTag.AddObjectData(store, "SereServ", rdo.lstSereServ);
                }
                result = true;
            }
            catch (Exception ex)
