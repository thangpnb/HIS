...
{
    class Mps000176Processor : ProcessorBase, IProcessorPrint
    {
        Mps000176RDO rdo;
        ...
        internal Mps000176Processor(SAR_PRINT_TYPE config, string fileName, object data, MPS.Printer.PreviewType previewType, string printerName)
            : base(config, (RDOBase)data, previewType, printerName)
        {
            rdo = (Mps000176RDO)data;
        ...
        internal void SetBarcodeKey()
        {
            try
            {
                ...
                dicImage.Add(Mps000176ExtendSingleKey.TREATMENT_CODE_BAR, barcodeTreatment);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
