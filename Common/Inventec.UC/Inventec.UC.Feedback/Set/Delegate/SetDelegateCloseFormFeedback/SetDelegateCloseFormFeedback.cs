...
{
    class SetDelegateCloseFormFeedback : ISetDelegateCloseFormFeedback
    {
        public bool SetDelegateClose(System.Windows.Forms.UserControl UC, CloseForm close)
        {
            bool result = false;
            try
            {
                if (UC.GetType() == typeof(Design.Template1.Template1))
                {
                    ...
                    result = UCFeedback.SetDelegateCloseForm(close);
                    if (result == false)
                    {
