...
{
    class SetDelegateGetValueComboProvince : ISetDelegateGetValueComboProvince
    {
        public bool SetDelegateGetValueProvince(System.Windows.Forms.UserControl UC, GetValueComboProvince GetValue)
        {
            bool result = false;
            try
            {
                if (UC.GetType() == typeof(Design.Template1.Template1))
                {
                    ...
                    result = UCComboCommune.SetDelegateGetValueProvince(GetValue);
                    if (result == false)
                    {
