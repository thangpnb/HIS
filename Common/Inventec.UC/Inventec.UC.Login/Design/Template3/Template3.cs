...
{
    internal partial class Template3 : UserControl
    {
        ...
        public Template3(InitUCD data)
        {
            ...
            {
                InitializeComponent();
                this.defaultLoginname = data != null ? data.LoginnameDefault : "";
                ...
                if (data.reloadLoginnameAfterLeave != null)
                {
        ...
        private void Template3_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!String.IsNullOrEmpty(this.defaultLoginname))
                {
                    if (this.reloadLoginnameAfterLeave != null)
                    {
                        this.reloadLoginnameAfterLeave(this.defaultLoginname);
                    }
                }
                else
                {
        ...
        private void ProcessIpLocal()
        {
            try
            {
                lblIP.Text = String.Format("Ip: {0}", Inventec.Common.WebApiClient.IpAddressUtils.GetIpAddressLocal());
            }
            catch (Exception ex)
            {
                LogSystem.Warn(ex);
        ...
        private void InitDataForm(long? firstBranchId, object branchs)
        {
            try
            {
                ...
                try
                {
                    long branchId = Convert.ToInt64((appFolder.GetValue(RegistryConstant.BRANCH_KEY) ?? 0).ToString());

                    if (branchId > 0 && this.CheckExistsBranchs(branchId, branchs))
                    {
                        cboBranch.EditValue = branchId;
                    }
                    else
                    {
        ...
        private bool CheckExistsBranchs(long id, object branchs)
        {
            bool result = false;
            try
            {
                if (branchs != null && branchs is IList)
                {
                    foreach (var item in (branchs as IList))
                    {
                        if (item == null) continue;
                        Type myType = item.GetType();
                        PropertyInfo p = myType.GetProperty("ID");
                        if ((long)p.GetValue(item) == id)
                            return true;
                    }
                }
            }
        ...
        private void txtLoginName_Leave(object sender, EventArgs e)
        {
            try
            {
                if (this.reloadLoginnameAfterLeave != null)
                {
                    this.reloadLoginnameAfterLeave(txtLoginName.Text);
                }
            }
            catch (Exception ex)
            {
