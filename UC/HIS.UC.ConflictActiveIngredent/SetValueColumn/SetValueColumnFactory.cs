...
{
    class SetValueColumnFactory
    {
        internal static ISetValueColumn MakeISetValueColumn(CommonParam param, UserControl uc, string Fieldname, string data)
        {
            ISetValueColumn result = null;
            try
            {
                if (uc is UserControl)
                {
                    result = new SetValueColumnBehavior(param, (UserControl)uc,Fieldname, data);
                }
