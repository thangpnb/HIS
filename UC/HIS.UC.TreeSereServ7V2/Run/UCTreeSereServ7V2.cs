...
namespace HIS.UC.TreeSereServ7V2.Run
{
    public partial class UCTreeSereServ7V2 : UserControl
    {
        ...
        TreeSereServ7_ReloadFilter treeSereServ7_ReloadFilter;
        ...
        List<MOS.EFMODEL.DataModels.L_HIS_SERVICE_REQ> currentServiceReqList;
        ...
        #region Construct
        public UCTreeSereServ7V2(TreeSereServ7ADO sereServTreeADO)
        {
            InitializeComponent();
            try
            {
                ...
                this.treeSereServ7_ReloadFilter = sereServTreeADO.TreeSereServ7_ReloadFilter;
        ...
        private void UCServiceTree_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (TreeSereServ7ADO != null)
                {
                    ...
                    if (!TreeSereServ7ADO.IsNotGroupServiceType)
                    {
                        LoadTreeListByFilter(true, this.TreeSereServ7ADO.SereServs);
                    }
                    else
                    {
                        LoadTreeListByFilterNotGroupServiceType(this.TreeSereServ7ADO.SereServs);
                    }
        ...
        private void InitControlState()
        {
            try
            {
                ...
                this.currentControlStateRDO = controlStateWorker.GetData("HIS.UC.TreeSereServ7V2");
                if (this.currentControlStateRDO != null && this.currentControlStateRDO.Count > 0)
                {
                    foreach (var item in this.currentControlStateRDO)
                    {
                        if (item.KEY == cboFilterByDepartment.Name)
                        {
                            cboFilterByDepartment.SelectedIndex = Convert.ToInt32(item.VALUE);
                        }

                    }
                }
                if (cboFilterByDepartment.SelectedIndex < 0)
                    cboFilterByDepartment.SelectedIndex = 0;
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                ...
                Resources.ResourceLanguageManager.LanguageResource = new ResourceManager("HIS.UC.TreeSereServ7V2.Resources.Lang", typeof(HIS.UC.TreeSereServ7V2.Run.UCTreeSereServ7V2).Assembly);
                this.cboFilterByDepartment.Properties.Items.AddRange(new object[] {
            Inventec.Common.Resource.Get.Value("UCTreeSereServ7V2.cboFilter2.Text", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture()),
            Inventec.Common.Resource.Get.Value("UCTreeSereServ7V2.cboFilter1.Text", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture()),
            "Tất cả (không bao gồm dịch vụ nội trú)",
            });
                ////Gan gia tri cho cac control editor co Text/Caption/ToolTip/NullText/NullValuePrompt/FindNullPrompt
                this.trvService.OptionsFind.FindNullPrompt = Inventec.Common.Resource.Get.Value("UCTreeSereServ7V2.trvService.OptionsFind.FindNullPrompt", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.layoutControl1.Text = Inventec.Common.Resource.Get.Value("UCTreeSereServ7V2.layoutControl1.Text", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.btnThuGon.Text = Inventec.Common.Resource.Get.Value("UCTreeSereServ7V2.btnThuGon.Text", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.btnChiTiet.Text = Inventec.Common.Resource.Get.Value("UCTreeSereServ7V2.btnChiTiet.Text", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        public void Reload(List<MOS.EFMODEL.DataModels.V_HIS_SERE_SERV_7> sereServs, List<MOS.EFMODEL.DataModels.L_HIS_SERVICE_REQ> _serviceReqList)
        {
            List<SereServADO> sereServsADO = new List<SereServADO>();
            AutoMapper.Mapper.CreateMap<V_HIS_SERE_SERV_7, SereServADO>();
            sereServsADO = AutoMapper.Mapper.Map<List<V_HIS_SERE_SERV_7>, List<SereServADO>>(sereServs);
            Reload(sereServsADO, _serviceReqList);
        }

        public void Reload(List<SereServADO> sereServs, List<MOS.EFMODEL.DataModels.L_HIS_SERVICE_REQ> _serviceReqList)
        {
            try
            {
                if (sereServs != null)
                ...
                this.currentServiceReqList = _serviceReqList;
                if (this.TreeSereServ7ADO.SereServs == null)
                    records = null;
                //BindTreePlus();
                if (!this.TreeSereServ7ADO.IsNotGroupServiceType)
                {
        ...
        public string GetHTPerTotal()
        {
            string str = "";
            try
            {
                var dtcount = SereServADOs.Where(o => o.SERVICE_REQ_STT_ID == IMSys.DbConfig.HIS_RS.HIS_SERVICE_REQ_STT.ID__HT && o.IsServiceParent == true).ToList();
                var dttotal = SereServADOs.Where(o => o.SERVICE_REQ_STT_ID > 0 && o.IsServiceParent == true).ToList();
                str = string.Format("({0}/{1})", dtcount != null && dtcount.Count > 0 ? dtcount.Count.ToString() : "0", dttotal.Count.ToString());
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        public List<SereServADO> GetListAll()
        {
            List<SereServADO> result = new List<SereServADO>(); ;
            try
            {
            ...
            try
            {
                if (e.SelectedControl is DevExpress.XtraTreeList.TreeList)// trvService)
                {
                    ...
                    if (hi.HitInfoType == HitInfoType.SelectImage)
                    {
                        ...
                        if (data != null && data.IsServiceParent == true)
                        {
        ...
        private void cboFilterByDepartment_Closed(object sender, DevExpress.XtraEditors.Controls.ClosedEventArgs e)
        {
            try
            {
                ...
                if (treeSereServ7_ReloadFilter != null)
                {
                    treeSereServ7_ReloadFilter();
                }

                btnThuGon.Visible = true;
                btnChiTiet.Visible = false;
                HIS.Desktop.Library.CacheClient.ControlStateRDO csAddOrUpdate = (this.currentControlStateRDO != null && this.currentControlStateRDO.Count > 0) ? this.currentControlStateRDO.Where(o => o.KEY == cboFilterByDepartment.Name && o.MODULE_LINK == "HIS.UC.TreeSereServ7V2").FirstOrDefault() : null;
                Inventec.Common.Logging.LogSystem.Debug(Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => csAddOrUpdate), csAddOrUpdate));
                if (csAddOrUpdate != null)
                {
                    csAddOrUpdate.VALUE = (cboFilterByDepartment.SelectedIndex != null ? cboFilterByDepartment.SelectedIndex.ToString() : "");
                }
                else
                {
                    csAddOrUpdate = new HIS.Desktop.Library.CacheClient.ControlStateRDO();
                    csAddOrUpdate.KEY = cboFilterByDepartment.Name;
                    csAddOrUpdate.VALUE = (cboFilterByDepartment.SelectedIndex != null ? cboFilterByDepartment.SelectedIndex.ToString() : "");
                    csAddOrUpdate.MODULE_LINK = "HIS.UC.TreeSereServ7V2";
                ...
                Inventec.Common.Logging.LogSystem.Warn("cboChange");
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
