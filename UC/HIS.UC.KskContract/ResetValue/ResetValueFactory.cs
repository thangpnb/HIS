...
{
    class ResetValueFactory
    {
        internal static IResetValue MakeIResetValue(CommonParam param, UserControl uc)
        {
            IResetValue result = null;
            try
            {
                if (uc is UserControl)
                {
                    result = new ResetValueBehavior(param, (UserControl)uc);
                }
