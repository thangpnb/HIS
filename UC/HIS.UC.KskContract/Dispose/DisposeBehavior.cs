...
namespace HIS.UC.KskContract.Dispose
{
    public sealed class DisposeBehavior : IDispose
    {
        ...
        void IDispose.Run(TemplateType.ENUM temp)
        {
            try
            {
                switch (temp)
                {
                    case TemplateType.ENUM.TEMPLATE_1:
                        ((UCKskContract__Template1)this.control).DisposeControl();
                        break;
                    case TemplateType.ENUM.TEMPLATE_2:
                        ((UCKskContract__Template2)this.control).DisposeControl();
                        break;
                    default:
                        break;
                };
