...
namespace HIS.UC.Icd.Reload
{
    class ReloadFactory
    {
        internal static IReload MakeIReload(CommonParam param, UserControl uc, IcdInputADO data, HIS.UC.Icd.ADO.Template template = HIS.UC.Icd.ADO.Template.Default)
        {
            IReload result = null;
            try
            {
                ...
                {
                    result = new ReloadBehavior(param, (UserControl)uc, data, template);
                }
