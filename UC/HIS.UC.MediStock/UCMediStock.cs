...
using HIS.UC.MediStock.Popup;
using DevExpress.XtraGrid.Views.Grid.ViewInfo;

namespace HIS.UC.MediStock
{
    public partial class UCMediStock : UserControl
    {
        MediStockInitADO initADO = null;

        Grid_CustomUnboundColumnData gridView_CustomUnboundColumnData = null;
        btn_Radio_Enable_Click1 btn_Radio_Enable_Click1 = null;
        gridViewMediStock_MouseDownMest gridViewMediStock_MouseDownMest = null;

        Grid_RowCellClick gridView_RowCellClick = null;

        GridView_MouseRightClick gridView_MouseRightClick = null;
        HIS.UC.MediStock.MediStockADO currentAdo;
        DevExpress.XtraBars.BarManager barManager1;
        PopupMenuProcessor popupMenuProcessor;

        bool isShowSearchPanel;

        public UCMediStock(MediStockInitADO ado)
        {
            InitializeComponent();
            try
            {
                this.initADO = ado;
                this.gridView_CustomUnboundColumnData = ado.MediStockGrid_CustomUnboundColumnData;
                this.gridView_RowCellClick = ado.ListMediStockGrid_RowCellClick;
                gridViewMediStock_MouseDownMest = ado.gridViewMediStock_MouseDownMest;
        ...
        public object GetDataGridView()
        {
            ...
            try
            {
                result = (List<HIS.UC.MediStock.MediStockADO>)gridControlMediStock.DataSource;
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        private void UpdateDataSource()
        {
            try
            {
                ...
                gridControlMediStock.DataSource = this.initADO.ListMediStock;
        ...
        private void ProcessColumn()
        {
            try
            {
                if (this.initADO.ListMediStockColumn != null && this.initADO.ListMediStockColumn.Count > 0)
                {
                    foreach (var item in this.initADO.ListMediStockColumn)
                    {
        ...
        internal void Reload(List<MediStockADO> data)
        {
            try
            {
                if (this.initADO != null)
                {
                    this.initADO.ListMediStock = data;
                    UpdateDataSource();
                }
            }
            catch (Exception ex)
        ...
        private void gridViewMediStock_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    var data = (MediStockADO)gridViewMediStock.GetRow(e.RowHandle);
                    if (data != null)
                    {
        ...
        private void repositoryItemRadio_Enable_Click(object sender, EventArgs e)
        {
            try
            {
                var row = (V_HIS_MEDI_STOCK)gridViewMediStock.GetFocusedRow();
                foreach (var item in this.initADO.ListMediStock)
                {
                    if (item.ID == row.ID)
                    {
                        item.radioMest = true;
        ...
        private void gridViewMediStock_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            try
            {
                var data = (V_HIS_MEDI_STOCK)gridViewMediStock.GetFocusedRow();
        ...
        private void gridView_PopupMenuShowing(object sender, PopupMenuShowingEventArgs e)
        {
            try
            {
                currentAdo = (HIS.UC.MediStock.MediStockADO)gridViewMediStock.GetFocusedRow();
