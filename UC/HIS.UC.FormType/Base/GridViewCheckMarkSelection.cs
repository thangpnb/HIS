...
namespace DevExpress.XtraGrid.Selection
{
    public class GridCheckMarksSelection
    {
        ...
        public GridCheckMarksSelection(GridView repository)
            : this()
        {
            CurrentRepository = repository;
        }

        public GridView CurrentRepository
        {
        ...
        protected virtual void Attach(GridView rep)
        {
            if (rep == null) return;
            selection.Clear();
            _currentRepository = rep;

            edit = _currentRepository.GridControl.RepositoryItems.Add("CheckEdit") as RepositoryItemCheckEdit;

            GridColumn column = _currentRepository.Columns.Add();
            ...
            _currentRepository.Click += new EventHandler(View_Click);
            _currentRepository.CustomDrawColumnHeader += new ColumnHeaderCustomDrawEventHandler(View_CustomDrawColumnHeader);
            _currentRepository.CustomDrawGroupRow += new RowObjectCustomDrawEventHandler(View_CustomDrawGroupRow);
            _currentRepository.CustomUnboundColumnData += new CustomColumnDataEventHandler(view_CustomUnboundColumnData);
            _currentRepository.KeyDown += new KeyEventHandler(view_KeyDown);
        }
        ...
        protected virtual void Detach()
        {
            ...
            if (edit != null)
            {
                _currentRepository.GridControl.RepositoryItems.Remove(edit);
                edit.Dispose();
            }
            _currentRepository.Click -= new EventHandler(View_Click);
            _currentRepository.CustomDrawColumnHeader -= new ColumnHeaderCustomDrawEventHandler(View_CustomDrawColumnHeader);
