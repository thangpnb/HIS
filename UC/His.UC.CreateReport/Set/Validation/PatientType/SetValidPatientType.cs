...
namespace His.UC.CreateReport.Set.Validation.PatientType
{
    class SetValidPatientType : ISetValidPatientType
    {
        void ISetValidPatientType.Set(System.Windows.Forms.UserControl UC)
        {
            try
            {
                if (UC != null)
                {
                    if (UC.GetType() == typeof(Design.TimeAndPatientType.TimeAndPatientType))
                    {
                        Design.TimeAndPatientType.TimeAndPatientType ucCreateReport = (Design.TimeAndPatientType.TimeAndPatientType)UC;
                        ucCreateReport.SetValidateCboPatientType();
                    }
