...
namespace HIS.UC.Room
{
    public partial class UCRoom : UserControl
    {
        ...
        HIS.UC.Room.RoomAccountADO currentRoom;
        PopupMenuProcessor.ItemType itemType;

        bool isShowSearchPanel;

        public UCRoom(RoomInitADO ado)
        {
            InitializeComponent();
            try
            {
        ...
        public object GetDataGridView()
        {
            ...
            try
            {
                result = (List<HIS.UC.Room.RoomAccountADO>)gridControlRoom.DataSource;
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
            ...
            try
            {
                if (this.initADO.ListRoomColumn != null && this.initADO.ListRoomColumn.Count > 0)
                {
                    foreach (var item in this.initADO.ListRoomColumn)
                    {
                        ...
                        if (item.image != null && item.Caption.Equals(" "))
                        {
                        ...
                        {
                            col.Image = item.image;
                            col.ImageAlignment = StringAlignment.Near;
                        }
        ...
        internal void Reload(List<RoomAccountADO> data)
        {
            try
            {
                if (this.initADO != null)
        ...
        internal void ReloadColumn(List<RoomColumn> data)
        {
            try
            {
                if (data != null && data.Count > 0)
                {
                    ...
                    {
                        foreach (GridColumn header in this.gridViewRoom.Columns)
                        {
                            if (item.FieldName == header.FieldName)
                            {
                                ...
                                if (item.image != null && item.Caption.Equals(" "))
                                {
                                    ...
                                    header.ImageAlignment = StringAlignment.Center;
                                }
                                if (item.image != null && !item.Caption.Equals(" "))
                                {
                                    header.Image = item.image;
                                    header.ImageAlignment = StringAlignment.Near;
                                }
        ...
        private void gridViewRoom_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    var data = (RoomAccountADO)gridViewRoom.GetRow(e.RowHandle);
                    if (data != null)
                    {
        ...
        private void repositoryItemRadio_Enable_Click(object sender, EventArgs e)
        {
            try
            {
                var row = (V_HIS_ROOM)gridViewRoom.GetFocusedRow();
        ...
        private void gridViewRoom_PopupMenuShowing_1(object sender, PopupMenuShowingEventArgs e)
        {
            try
            {
                currentRoom = (HIS.UC.Room.RoomAccountADO)gridViewRoom.GetFocusedRow();
