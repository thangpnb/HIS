...
namespace HIS.UC.TreatmentFinish
{
    class CommonBaseEditor
    {
        ...
        internal static void ValidationControlDeathCauseDetail(BaseEdit control, IsValidControl isValidControl, DevExpress.XtraEditors.DXErrorProvider.DXValidationProvider dxValidationProviderEditor)
        {
            try
            {
                ControlEditValidationRule validRule = new ControlEditValidationRule();
                ...
                else if (Inventec.Common.String.CountVi.Count(control.Text) > 2000)
                {
                    validRule.ErrorText = ResourceMessage.TruongThongTinCoDoDaiVuotQuaGioiHan;
                }
