...
namespace HIS.UC.TreatmentFinish.EndTypeForm
{
    public partial class FormAppointment : HIS.Desktop.Utility.FormBase
    {
        ...
        public FormAppointment(HIS_TREATMENT treatment, HisTreatmentFinishSDO treatmentFinishSDO, DataInputADO dataInputADO, TreatmentFinishInitADO _treatmentFinishInitADO, Action<HisTreatmentFinishSDO> _actEdited, Action<decimal> _actGetUseDay,bool? IsBlockOrder=null)
        {
            InitializeComponent();
            try
            {
                this.currentTreatmentFinishSDO = treatmentFinishSDO;
                this.currentDataInputADO = dataInputADO;
                this.hisTreatment = treatment;
                this.actEdited = _actEdited;
                this.actGetUseDay = _actGetUseDay;
                this.treatmentFinishInitADO = _treatmentFinishInitADO;
                this.TreatmentEndAppointmentTimeDefault = _treatmentFinishInitADO.TreatmentEndAppointmentTimeDefault;
                this.TreatmentEndHasAppointmentTimeDefault = _treatmentFinishInitADO.TreatmentEndHasAppointmentTimeDefault;
        ...
        private void FormAppointment_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                Inventec.Common.Logging.LogSystem.Debug(Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => ConfigKeyCFG.NumOrderIssueOption), ConfigKeyCFG.NumOrderIssueOption));
                if (IsBlockOrder == true)
                {
                ...
                else
                {
                    ...
                    InitCboTimeFrame();
                    if (ConfigKeyCFG.NumOrderIssueOption == "1")
                    {
        ...
        private void InitCboTimeFrame()
        {
            try
            {
                ...
                else if (ConfigKeyCFG.NumOrderIssueOption == "2")
                {
                    layoutControlItem8.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Always;
        ...
        private void ProcessLoadGetOccupiedStatus()
        {
            try
            {
                Inventec.Common.Logging.LogSystem.Debug("ProcessLoadGetOccupiedStatus.1");
                cboTimeFrame.EditValue = null;
                resultChooseNumOrderBlockADO = null;
        ...
        private void SetAppoinmentTime()
        {
            try
            {
                if (this.currentTreatmentFinishSDO != null && this.currentTreatmentFinishSDO.AppointmentTime > 0)
                {
                    Inventec.Common.Logging.LogSystem.Debug("SetAppoinmentTime.1");
                    Inventec.Common.Logging.LogSystem.Debug(Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => currentTreatmentFinishSDO), currentTreatmentFinishSDO));
                    dtTimeAppointments.DateTime = Inventec.Common.DateTime.Convert.TimeNumberToSystemDateTime(this.currentTreatmentFinishSDO.AppointmentTime ?? 0) ?? DateTime.MinValue;
                }
                else
                {
                    Inventec.Common.Logging.LogSystem.Debug("SetAppoinmentTime.2");
                    Inventec.Common.Logging.LogSystem.Debug(Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => TreatmentEndHasAppointmentTimeDefault), TreatmentEndHasAppointmentTimeDefault) +
                        Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => TreatmentEndAppointmentTimeDefault), TreatmentEndAppointmentTimeDefault));
                    if (!TreatmentEndHasAppointmentTimeDefault && TreatmentEndAppointmentTimeDefault > 0)
                    {
                        dtTimeAppointments.DateTime = Inventec.Common.DateTime.Convert.TimeNumberToSystemDateTime(
                                Inventec.Common.DateTime.Calculation.Add(this.currentDataInputADO.UseTime ?? 0,
                    ...
                    {
                        DateTime useTimeTo = Inventec.Common.DateTime.Convert.TimeNumberToSystemDateTime(this.currentDataInputADO.UseTimeTo ?? 0)
                               ?? DateTime.Now;
                        dtTimeAppointments.DateTime = useTimeTo.AddDays((long)1);
                    }
                }

                DateTime appointmentTime = dtTimeAppointments.DateTime;
                DateTime dtUseTime = Inventec.Common.DateTime.Convert.TimeNumberToSystemDateTime(this.currentDataInputADO.UseTime ?? 0) ?? DateTime.Now;
                TimeSpan ts = (TimeSpan)(appointmentTime.Date - dtUseTime.Date);
                spDay.Value = (decimal)(ts.TotalDays);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
        ...
        private void LoadKeysFromlanguage()
        {
            try
            {
                ...
                this.cboRoomEx.Properties.NullText = Inventec.Common.Resource.Get.Value("FormAppointment.cboRoomEx.Properties.NullText", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                ...
                this.bbtnChonKhungGio.ToolTip = Inventec.Common.Resource.Get.Value("FormAppointment.bbtnChonKhungGio.ToolTip", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.btnServiceAppoint.Text = Inventec.Common.Resource.Get.Value("FormAppointment.btnServiceAppoint.Text", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.btnServiceAppoint.ToolTip = Inventec.Common.Resource.Get.Value("FormAppointment.btnServiceAppoint.ToolTip", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.cboTimeFrame.Properties.NullText = Inventec.Common.Resource.Get.Value("FormAppointment.cboTimeFrame.Properties.NullText", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.txtAdvise.Properties.NullValuePrompt = Inventec.Common.Resource.Get.Value("FormAppointment.txtAdvise.Properties.NullValuePrompt", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.txtSearch.Properties.NullValuePrompt = Inventec.Common.Resource.Get.Value("FormAppointment.txtSearch.Properties.NullValuePrompt", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.gridViewRoomExam.OptionsFind.FindNullPrompt = Inventec.Common.Resource.Get.Value("FormAppointment.gridViewRoomExam.OptionsFind.FindNullPrompt", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.gridColumn2.Caption = Inventec.Common.Resource.Get.Value("FormAppointment.gridColumn2.Caption", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.gridColumn3.Caption = Inventec.Common.Resource.Get.Value("FormAppointment.gridColumn3.Caption", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                ...
                this.layoutControlItem4.OptionsToolTip.ToolTip = Inventec.Common.Resource.Get.Value("FormAppointment.layoutControlItem4.OptionsToolTip.ToolTip", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.layoutControlItem4.Text = Inventec.Common.Resource.Get.Value("FormAppointment.layoutControlItem4.Text", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.layoutControlItem5.Text = Inventec.Common.Resource.Get.Value("FormAppointment.layoutControlItem5.Text", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.layoutControlItem6.Text = Inventec.Common.Resource.Get.Value("FormAppointment.layoutControlItem6.Text", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.layoutControlItem1.Text = Inventec.Common.Resource.Get.Value("FormAppointment.layoutControlItem1.Text", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.lciTimeFrame.Text = Inventec.Common.Resource.Get.Value("FormAppointment.lciTimeFrame.Text", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.layoutControlItem9.Text = Inventec.Common.Resource.Get.Value("FormAppointment.layoutControlItem9.Text", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.layoutControlItem10.Text = Inventec.Common.Resource.Get.Value("FormAppointment.layoutControlItem10.Text", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.lblStt.Text = Inventec.Common.Resource.Get.Value("FormAppointment.lblStt.Text", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
                this.layoutControlItem11.Text = Inventec.Common.Resource.Get.Value("FormAppointment.layoutControlItem11.Text", Resources.ResourceLanguageManager.LanguageFormAppointment, LanguageManager.GetCulture());
        ...
        private void SetDefaultValueControl()
        {
            try
            {
                Inventec.Common.Logging.LogSystem.Debug(Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => hisTreatment), hisTreatment) +
                    Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => currentTreatmentFinishSDO), currentTreatmentFinishSDO)
                    + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => currentDataInputADO), currentDataInputADO));

                if (dtTimeAppointments.Enabled == false)
                    return;

                if (!this.currentDataInputADO.UseTime.HasValue || !this.currentDataInputADO.UseTimeTo.HasValue)
                {
                    ...
                    && _RoomExamADOs != null && _RoomExamADOs.Count > 0)
                {
                    ...
                    foreach (var item in _RoomExamADOs)
                    {
                        ...
                        {
                            item.IsCheck = true;
                            ConfigKeyCFG.NumOrderIssueOption = item.IS_BLOCK_NUM_ORDER == 1 ? "2" : "1";
                            if (ConfigKeyCFG.NumOrderIssueOption == "2")
                            {
                                break;
                            }
                        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!ValidateMustChooseSeviceInCaseOfAppointment())
                    return;

                //Kiểm tra nếu số ngày hẹn khám vượt quá số ngày được cấu hình
                if (ConfigKeyCFG.MaxOfAppointmentDays.HasValue && spDay.Value > ConfigKeyCFG.MaxOfAppointmentDays.Value)
                {
                    //Tùy chọn xử lý trong trường hợp vượt quá số ngày hẹn khám. 1: Cảnh báo. 2: Chặn, không cho xử trí
                    if (ConfigKeyCFG.WarningOptionWhenExceedingMaxOfAppointmentDays == 2)
                    {
                        DevExpress.XtraEditors.XtraMessageBox.Show(string.Format(Resources.ResourceMessage.CanhBaoNgayHenToiDa, ConfigKeyCFG.MaxOfAppointmentDays),
                        Inventec.Desktop.Common.LibraryMessage.MessageUtil.GetMessage(Inventec.Desktop.Common.LibraryMessage.Message.Enum.TieuDeCuaSoThongBaoLaThongBao),
                        MessageBoxButtons.OK);
                        if (actEdited != null) actEdited(null);
                        return;
                    }
                    else if (ConfigKeyCFG.WarningOptionWhenExceedingMaxOfAppointmentDays == 1)
                    {
                        if (DevExpress.XtraEditors.XtraMessageBox.Show(string.Format("{0} Bạn có muốn tiếp tục không?", string.Format(Resources.ResourceMessage.CanhBaoNgayHenToiDa, ConfigKeyCFG.MaxOfAppointmentDays)),
                        Inventec.Desktop.Common.LibraryMessage.MessageUtil.GetMessage(Inventec.Desktop.Common.LibraryMessage.Message.Enum.TieuDeCuaSoThongBaoLaThongBao),
                ...
                Inventec.Common.Logging.LogSystem.Info("ConfigKeyCFG.AutoCreateWhenAppointment: "+ ConfigKeyCFG.AutoCreateWhenAppointment +"_____" + Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => hisTreatment), hisTreatment));

                if (ConfigKeyCFG.AutoCreateWhenAppointment == "1" && hisTreatment != null && hisTreatment.TDL_PATIENT_TYPE_ID == ConfigKeyCFG.PatientTypeId__BHYT)
                {
                    long dtAppointmentTime1 = Inventec.Common.DateTime.Convert.SystemDateTimeToTimeNumber(dtTimeAppointments.DateTime.Date) ?? 0;

                    long CardtoTime = hisTreatment.TDL_HEIN_CARD_TO_TIME ?? 0;
                    if (dtAppointmentTime1 > CardtoTime)
                    {
                        string T1 = Inventec.Common.DateTime.Convert.TimeNumberToDateString(CardtoTime);
                        string T2 = Inventec.Common.DateTime.Convert.TimeNumberToDateString(dtAppointmentTime);

                        if (DevExpress.XtraEditors.XtraMessageBox.Show(String.Format(Resources.ResourceMessage.TuDongDoiSangDoiTuong, T1, T2, ConfigKeyCFG.PatientTypeName__HospitalFee),
                                           Inventec.Desktop.Common.LibraryMessage.MessageUtil.GetMessage(Inventec.Desktop.Common.LibraryMessage.Message.Enum.TieuDeCuaSoThongBaoLaThongBao),
                ...
                if (dtAppointmentTime >= dtOutTime)
                {
                    ...
                    if (actGetUseDay != null)
                        this.actGetUseDay(spDay.Value);
                    this.Close();
                }
                else
                {
        ...
        bool ValidateMustChooseSeviceInCaseOfAppointment()
        {
            bool result = true;
            try
            {
                Inventec.Common.Logging.LogSystem.Debug("ValidateMustChooseSeviceInCaseOfAppointment.1");
                Inventec.Common.Logging.LogSystem.Debug(Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => ConfigKeyCFG.MustChooseSeviceInCaseOfAppointment), ConfigKeyCFG.MustChooseSeviceInCaseOfAppointment));
                if (ConfigKeyCFG.MustChooseSeviceInCaseOfAppointment)
                {
                    Inventec.Common.Logging.LogSystem.Debug("ValidateMustChooseSeviceInCaseOfAppointment.2");
                    CommonParam param = new CommonParam();
                    ...
                    {
                        Inventec.Common.Logging.LogSystem.Debug("ValidateMustChooseSeviceInCaseOfAppointment.3");
                        DevExpress.XtraEditors.XtraMessageBox.Show(Resources.ResourceMessage.BanBatBuocChonDichVuHenKhamKhiKetThucDieuTri,
                                           Inventec.Desktop.Common.LibraryMessage.MessageUtil.GetMessage(Inventec.Desktop.Common.LibraryMessage.Message.Enum.TieuDeCuaSoThongBaoLaThongBao));

                        Inventec.Common.Logging.LogSystem.Warn(Resources.ResourceMessage.BanBatBuocChonDichVuHenKhamKhiKetThucDieuTri);
                        result = false;
                    }
                    Inventec.Common.Logging.LogSystem.Debug("ValidateMustChooseSeviceInCaseOfAppointment.4");
                }
                Inventec.Common.Logging.LogSystem.Debug("ValidateMustChooseSeviceInCaseOfAppointment.5");
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
            ...
            try
            {
                ...
                if (executeRoomIds != null && executeRoomIds.Count > 0)
                {
                    ...
                    if (overs != null && overs.Count > 0)
                    {
                        ...
                        if (XtraMessageBox.Show(mess, Inventec.Desktop.Common.LibraryMessage.MessageUtil.GetMessage(Inventec.Desktop.Common.LibraryMessage.Message.Enum.TieuDeCuaSoThongBaoLaThongBao), MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) != System.Windows.Forms.DialogResult.Yes)
                            return false;
                    }
                }
            }
                ...
                else
                {
                    if (!editcboTimeFrame)
                    {
                        ...
                        if (editor != null)
                        {
                            ...
                            if (editor.OldEditValue != null && ConfigKeyCFG.NumOrderIssueOption == "1")
                            {
        ...
        private void CalculateDayNum()
        {
            try
            {
                if (dtTimeAppointments.EditValue != null)
                {
                    ...
                    else if (ConfigKeyCFG.NumOrderIssueOption == "2" && !initNumOderBlock)
                    {
                        ProcessLoadGetOccupiedStatus();
                    }
                }
        ...
        private void cboTimeFrame_CustomDisplayText(object sender, DevExpress.XtraEditors.Controls.CustomDisplayTextEventArgs e)
        {
            try
            {
                if (cboTimeFrame.EditValue != null && ConfigKeyCFG.NumOrderIssueOption == "1")
                {
                ...
                else if (cboTimeFrame.EditValue != null && ConfigKeyCFG.NumOrderIssueOption == "2")
                {
        ...
        private void gridViewRoomExam_CellValueChanged(object sender, CellValueChangedEventArgs e)
        {
            try
            {
                ...
                if (dataRow.IsCheck)
                {
                    string currentNumOrderIssueOption = ConfigKeyCFG.NumOrderIssueOption;
                    ConfigKeyCFG.NumOrderIssueOption = (dataRow != null && dataRow.IS_BLOCK_NUM_ORDER == 1) ? "2" : "1";
                    ...
                    Inventec.Common.Logging.LogSystem.Debug(Inventec.Common.Logging.LogUtil.TraceData(Inventec.Common.Logging.LogUtil.GetMemberName(() => currentNumOrderIssueOption), currentNumOrderIssueOption)
                        + Inventec.Common.Logging.LogUtil.TraceData("ConfigKeyCFG.NumOrderIssueOption", ConfigKeyCFG.NumOrderIssueOption));

                    if (ConfigKeyCFG.AutoCreateWhenAppointment == "1")
                    {
                        _RoomExamADOs.ForEach(k => k.IsCheck = false);
                    ...
                    bool isChangeConfig = (currentNumOrderIssueOption != ConfigKeyCFG.NumOrderIssueOption);
                    if (isChangeConfig)
                    {
                        this.initNumOderBlock = true;
                        InitCboTimeFrame();
                    ...
                    else if (ConfigKeyCFG.NumOrderIssueOption == "1")
                    {
                        editdtTimeAppointments = true;
                        cboTimeFrame.EditValue = ProcessGetFromTime(dtTimeAppointments.DateTime);
        ...
        private void FormAppointment_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                if (!IsReturnClosed)
                {
                    ConfigKeyCFG.NumOrderIssueOption = "1";
                }
            }
            catch (Exception ex)
            {
