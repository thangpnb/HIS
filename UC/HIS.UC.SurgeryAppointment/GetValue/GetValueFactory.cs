...
namespace HIS.UC.SurgeryAppointment.GetValue
{
    ...
    {
        internal static IGetValue MakeIGetValue(CommonParam param, UserControl uc, bool isPlus = false)
        {
            ...
            try
            {
                if (!isPlus)
                {
                    result = new GetValueBehavior(param, uc);
                }
                else
                {
                    result = new GetValuePlusBehavior(param, uc);
                }
