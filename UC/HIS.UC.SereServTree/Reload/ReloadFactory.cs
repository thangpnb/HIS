...
namespace HIS.UC.SereServTree.Reload
{
    class ReloadFactory
    {
        ...
        internal static IReload MakeIReloadSereServDeposit(CommonParam param, UserControl data, List<MOS.EFMODEL.DataModels.V_HIS_SERE_SERV_DEPOSIT> sereServDeposits)
        {
            IReload result = null;
            try
            {
                if (data is UserControl)
                {
                    result = new ReloadBehavior(param, (UserControl)data, sereServDeposits);
                }
                if (result == null) throw new NullReferenceException();
            }
            catch (NullReferenceException ex)
