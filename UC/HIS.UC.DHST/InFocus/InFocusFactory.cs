...
namespace HIS.UC.DHST.InFocus
{
    class InFocusFactory
    {
        internal static IInFocus MakeIInFocus(CommonParam param, UserControl uc)
        {
            IInFocus result = null;
            try
            {
                if (uc is UserControl)
                {
                    result = new InFocusBehavior(param, (UserControl)uc);
                }
