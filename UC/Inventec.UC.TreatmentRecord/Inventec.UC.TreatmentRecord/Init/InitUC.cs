...
namespace Inventec.UC.TreatmentRecord.Init
{
    class InitUC : IInitUC
    {
        ...
        System.Windows.Forms.UserControl IInitUC.Init(MainListTreatment.EnumTemplate template)
        {
            ...
            try
            {
                ...
                if (valid)
                {
                    if (template == MainListTreatment.EnumTemplate.TEMPLATE1)
                    {
                        result = new UCTreatmentList(Data);
                    }
