...
namespace HIS.UC.ViewEmrDocument.UcEmrDocument
{
    public partial class UcViewEmrDocument : UserControl
    {
        ...
        public void ReloadDocument(List<EmrDocumentFileSDO> documentFileSDOs)
        {
            try
            {
                if (documentFileSDOs == null)
                    documentFileSDOs = new List<EmrDocumentFileSDO>();
                List<MemoryStream> documentData = new List<MemoryStream>();
                foreach (var item in documentFileSDOs)
                {
                    if (item.Extension.ToLower().Equals("pdf"))
                        documentData.Add(new MemoryStream(Convert.FromBase64String(item.Base64Data)));
                }
                if(documentData.Count > 0)
                    ProcessJoinDocument(documentData);
                else
                    ProcessJoinDocument(null);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
