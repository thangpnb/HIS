...
namespace HIS.UC.UCPatientRaw.Valid
{
	...
	{
		internal DevExpress.XtraEditors.LookUpEdit cboGenderCode;
	...
	class Valid_Cbo_Control : DevExpress.XtraEditors.DXErrorProvider.ValidationRule
	{
		...
		public override bool Validate(Control control, object value)
		{
			...
			try
			{
				if (cbo == null)
					return valid;
				if (cbo.Visible && (cbo.EditValue == null || String.IsNullOrWhiteSpace(cbo.Text)))
					return valid;
				valid = true;
			}
			catch (Exception ex)
	...
	class Valid_Txt_Control : DevExpress.XtraEditors.DXErrorProvider.ValidationRule
	{
		...
		public override bool Validate(Control control, object value)
		{
			...
			try
			{
				if (txt == null)
					return valid;
				if (txt.Visible && (txt.EditValue == null || String.IsNullOrWhiteSpace(txt.Text)))
					return valid;			
				valid = true;
			}
			catch (Exception ex)
