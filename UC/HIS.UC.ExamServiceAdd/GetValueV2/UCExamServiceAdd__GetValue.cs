...
namespace HIS.UC.ExamServiceAdd.Run
{
    ...
    {
        public ExamServiceAddADO GetValueV2()
        {
            ExamServiceAddADO result = null;
            try
            {
                this.positionHandle = -1;
                if (IsAllowNotChooseService && cboExamService.EditValue != null && cboPatientType.EditValue == null)
                {
                    DevExpress.XtraEditors.XtraMessageBox.Show("Dịch vụ khám thêm chưa nhập đối tượng thanh toán.",
                ...
                if (dxValidationProvider1.Validate())
                {
                    ...
                    if (chkKhongHuongBHYT.Checked) result.IsNotUseBhyt = true;
                    ...
                    {
                        result.FinishTime = Inventec.Common.DateTime.Convert.SystemDateTimeToTimeNumber(dtFinishTime.DateTime) ?? 0;
                    }
                    ...
                    {
                        result.IsPrintAppointment = true;
                    }

                    if (dtAppointment.Enabled && dtAppointment.EditValue != null && dtAppointment.DateTime != DateTime.MinValue)
                    {
                        result.AppointmentTime = Inventec.Common.DateTime.Convert.SystemDateTimeToTimeNumber(dtAppointment.DateTime) ?? 0;
                    }

                    if (txtAdvise.Enabled)
                    {
                        result.Advise = txtAdvise.Text;
                    }
                    if (NUM_ORDER_BLOCK_ID != null && NUM_ORDER_BLOCK_ID > 0)
                    {
                        result.NumOrderBlockId = NUM_ORDER_BLOCK_ID;
                    }
                    result.IsPrintExamAdd = chkPrintExamAdd.Checked;
                    result.IsSignExamAdd = chkSignExamAdd.Checked;

                    result.RoomApointmentId = cboRoomAppointment.EditValue != null ? (long?)Inventec.Common.TypeConvert.Parse.ToInt64(cboRoomAppointment.EditValue.ToString()) : null;

                    result.ServiceApointmentId = cboServiceAppointment.EditValue != null ? (long?)Inventec.Common.TypeConvert.Parse.ToInt64(cboServiceAppointment.EditValue.ToString()) : null;
                    result.Note = memNote.Text.Trim();
                }
            }
            catch (Exception ex)
            {
