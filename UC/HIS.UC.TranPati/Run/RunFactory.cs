...
namespace HIS.UC.TranPati.Run
{
    class RunFactory
    {
        internal static IRun MakeITranPati(CommonParam param, object data)
        {
            IRun result = null;
            try
            {
                if (data is ADO.TranPatiInitADO)
                {
                    result = new RunBehavior(param, (ADO.TranPatiInitADO)data);
                }
