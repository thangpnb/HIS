...
{
    public partial class UCFatherName : UserControlBase
    {
        #region Declare
            ...
            : base("UCPlusInfo", "UCFatherName")
        {
            try
            {
                ...
                this.txtFatherName.TabIndex = this.TabIndex;
                SetMaxlength(txtFatherName, 100);
            }
            catch (Exception ex)
            {
        ...
        private void UCFatherName_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                this.lciFatherName.Text = Inventec.Common.Resource.Get.Value("UCPlusInfo.UCFatherName", HIS.UC.PlusInfo.ShareMethod.ResourceLanguageManager.ResourceUCPlusInfo, LanguageManager.GetCulture());
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        /// <summary>
        ///Hàm xét ngôn ngữ cho giao diện UCFatherName
        /// </summary>
        private void SetCaptionByLanguageKeyNew()
        {
            try
            {
                ...
                ResourceLanguageManager.LanguageResource = new ResourceManager("HIS.UC.PlusInfo.Resources.Lang", typeof(UCFatherName).Assembly);

                ////Gan gia tri cho cac control editor co Text/Caption/ToolTip/NullText/NullValuePrompt/FindNullPrompt
                this.layoutControl1.Text = Inventec.Common.Resource.Get.Value("UCFatherName.layoutControl1.Text",ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.lciFatherName.OptionsToolTip.ToolTip = Inventec.Common.Resource.Get.Value("UCFatherName.lciFatherName.OptionsToolTip.ToolTip", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
                this.lciFatherName.Text = Inventec.Common.Resource.Get.Value("UCFatherName.lciFatherName.Text", ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        internal string GetValue()
        {
            try
            {
                return this.txtFatherName.Text;
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        internal void SetValue(string FatherName)
        {
            try
            {
                this.txtFatherName.Text = FatherName;
                //this.txtFatherName.TabIndex = this.TabIndex;
            }
            catch (Exception ex)
            {
