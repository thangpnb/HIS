...
{
    class ReloadRow1Factory
    {
        internal static IReloadRow1 MakeIReloadRow(CommonParam param, UserControl uc, HIS_ANTIGEN_METY data)
        {
            IReloadRow1 result = null;
            try
            {
                if (uc is UserControl)
                {
                    result = new ReloadRow1Behavior(param, (UserControl)uc, data);
                }
