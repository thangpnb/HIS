...
{
    class ValidationServiceUnitFactory
    {
        internal static IValidationServiceUnit MakeIValidationServiceUnit(CommonParam param, UserControl uc)
        {
            IValidationServiceUnit result = null;
            try
            {
                if (uc is UserControl)
                {
                    result = new ValidationServiceUnitBehavior(param, (UserControl)uc);
                }
