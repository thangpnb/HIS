...
namespace HIS.Desktop.Plugins.LocationTreatment
{
    public partial class frmLocationTreatment : FormBase
    {
        ...
        private void LoadDataTocboDataStore()
        {
            try
            {
                listDataStore = BackendDataWorker.Get<HIS_DATA_STORE>().Where(o => o.IS_ACTIVE == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE).ToList();
                ...
                ControlEditorLoader.Load(cboDataStore, listDataStore, controlEditorADO);
            }
            catch (Exception ex)
            {
        ...
        private void UpdateDTOFromDataForm(ref HIS_LOCATION_STORE currentDTO)
        {
            try
            {
                ...
                {
                    currentDTO.DATA_STORE_ID = Convert.ToInt64(cboDataStore.EditValue.ToString());
                }
            }
            catch (Exception ex)
            {
        ...
        private void cboDataStore_Closed(object sender, DevExpress.XtraEditors.Controls.ClosedEventArgs e)
        {
            try
            {
                if (e.CloseMode == PopupCloseMode.Normal)
                {
                    if (cboDataStore.EditValue != null)
                    {
                        var dataStore = listDataStore.SingleOrDefault(o => o.ID == Inventec.Common.TypeConvert.Parse.ToInt64((cboDataStore.EditValue ?? "").ToString()));
                        if (dataStore != null)
                        {
                            txtDataStoreCode.Text = dataStore.DATA_STORE_CODE;
        ...
        private void LoadDataStore(string _DataStoreCode)
        {
            try
            {
                ...
                listResult = this.listDataStore.Where(o => (!String.IsNullOrWhiteSpace(o.DATA_STORE_CODE) && o.DATA_STORE_CODE.StartsWith(_DataStoreCode))).ToList();
        ...
        private void cboDataStore_Properties_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
