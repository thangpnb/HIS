...
{
    public partial class frmInteractionReason : HIS.Desktop.Utility.FormBase
    {
        #region ---Decalre
        ...
        int dataTotal = 0;
        int rowCount = 0;
        HIS_INTERACTION_REASON CurrentData;
        RefeshReference dlgRefresh;
        #endregion
        public frmInteractionReason()
        {
            InitializeComponent();
        }
        public frmInteractionReason(Module module, RefeshReference dlg)
            : base(module)
        {
            try
            {
        ...
        private void frmHisAccidentBodyPart_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                ValidateText(txtHisInteractionReasonCode, 10);
                ValidateText(txtHisInteractionReasonName, 1000);
            }
            catch (Exception ex)
            {
        ...
        private void SetDefautData()
        {
            try
            {
                ...
                txtHisInteractionReasonCode.Text = "";
                txtHisInteractionReasonName.Text = "";
                txtSearch.Text = "";
                txtSearch.Focus();
            }
            catch (Exception ex)
        ...
        private void ChangeDataRow(HIS_INTERACTION_REASON datarow)
        {
            try
            {
                if (datarow != null)
        ...
        private void FillDataEditorControl(HIS_INTERACTION_REASON datarow)
        {
            try
            {
                
                txtHisInteractionReasonCode.Text = datarow.INTERACTION_REASON_CODE;
                txtHisInteractionReasonName.Text = datarow.INTERACTION_REASON_NAME;
            }
            catch (Exception ex)
            {
        ...
        private void UpdateDataFromform(HIS_INTERACTION_REASON updateDTO)
        {
            try
            {
                updateDTO.INTERACTION_REASON_CODE = txtHisInteractionReasonCode.Text.Trim();
                updateDTO.INTERACTION_REASON_NAME = txtHisInteractionReasonName.Text.Trim();
            }
            catch (Exception ex)
            {
                ...
                try
                {
                    foreach (DevExpress.XtraLayout.BaseLayoutItem item in lcEditorInfo.Items)
                    {
                        ...
                        if (lci != null && lci.Control != null && lci.Control is DevExpress.XtraEditors.BaseEdit)
                        {
                            ...
                            txtHisInteractionReasonCode.Focus();
                            txtHisInteractionReasonCode.SelectAll();
                        }
                    }
        ...
        private void SaveProcessor()
        {
            try
            {
                ...
                HIS_INTERACTION_REASON UpdateDTO = new HIS_INTERACTION_REASON();
                UpdateDataFromform(UpdateDTO);
                if (this.ActionType == GlobalVariables.ActionAdd)
                {
                    var ResultData = new BackendAdapter(param).Post<HIS_INTERACTION_REASON>(HisRequestUriStore.HisInteractionReason_Create, ApiConsumers.MosConsumer, UpdateDTO, param);
                    if (ResultData != null)
                    {
                        BackendDataWorker.Reset<HIS_INTERACTION_REASON>();
                        FillDataToGridControl();
                        success = true;
                        RestFromData();
                        if (dlgRefresh != null)
                ...
                else
                {
                    if (CurrentData != null)
                    {
                        ...
                        var ResultData = new BackendAdapter(param).Post<HIS_INTERACTION_REASON>(HisRequestUriStore.HisInteractionReason_Update, ApiConsumers.MosConsumer, UpdateDTO, param);
                        if (ResultData != null)
                        {
                            BackendDataWorker.Reset<HIS_INTERACTION_REASON>();
                            FillDataToGridControl();
        ...
        private void LoadPaging(object commonParam)
        {
            try
            {
                ...
                ApiResultObject<List<HIS_INTERACTION_REASON>> apiResult = null;
                HisInteractionReasonFilter filter = new HisInteractionReasonFilter();
               ...
               // SetFilter(ref filter);
                ...
                apiResult = new BackendAdapter(paramcommon).GetRO<List<HIS_INTERACTION_REASON>>(HisRequestUriStore.HisInteractionReason_Get, ApiConsumers.MosConsumer, filter, paramcommon);
                ...
                if (apiResult != null)
                {
                    var data = (List<HIS_INTERACTION_REASON>)apiResult.Data;
        ...
        private void SetFilter(ref HisInteractionReasonFilter filter)
        {
            try
            {
                filter.ORDER_DIRECTION = "DESC";
        ...
        private void gridViewAccidentBodyPart_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    var data = (HIS_INTERACTION_REASON)gridViewHisVaccineType.GetRow(e.RowHandle);
        ...
        private void gridViewAccidentBodyPart_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            try
            {
                ...
                {
                    HIS_INTERACTION_REASON DataRow = (HIS_INTERACTION_REASON)((IList)((BaseView)sender).DataSource)[e.ListSourceRowIndex];
                    if (DataRow != null)
                    {
                        DevExpress.XtraGrid.Views.Grid.GridView view = sender as DevExpress.XtraGrid.Views.Grid.GridView;
        ...
        private void gridViewAccidentBodyPart_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            try
            {
                ...
                if (e.RowHandle >= 0)
                {
                    var dataRow = (HIS_INTERACTION_REASON)((IList)((BaseView)sender).DataSource)[e.RowHandle];
                    if (dataRow != null)
                    {
        ...
        private void gridViewAccidentBodyPart_Click(object sender, EventArgs e)
        {
            try
            {
                var datarow = (HIS_INTERACTION_REASON)gridViewHisVaccineType.GetFocusedRow();
                if (datarow != null)
                {
        ...
        private void txtAccidentBodyPartCode_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            try
            {
                ...
                {
                    txtHisInteractionReasonName.Focus();
                    txtHisInteractionReasonName.SelectAll();
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnLock_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            {
                CommonParam param = new CommonParam();
                var rowData = (HIS_INTERACTION_REASON)gridViewHisVaccineType.GetFocusedRow();
                bool success = false;
                if (MessageBox.Show(LibraryMessage.MessageUtil.GetMessage(LibraryMessage.Message.Enum.HeThongTBCuaSoThongBaoBanCoMuonKhoaDuLieuKhong), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (rowData != null)
                    {
                        HIS_INTERACTION_REASON Result = new BackendAdapter(param).Post<HIS_INTERACTION_REASON>(HisRequestUriStore.HisInteractionReason_Changelock, ApiConsumer.ApiConsumers.MosConsumer, rowData.ID, param);
                        if (Result!=null)
                        {
                            BackendDataWorker.Reset<HIS_INTERACTION_REASON>();
                            success = true;
        ...
        private void btnDelete_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show(LibraryMessage.MessageUtil.GetMessage(LibraryMessage.Message.Enum.HeThongTBCuaSoThongBaoBanCoMuonXoaDuLieuKhong), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (rowData != null)
                    {
                        success = new BackendAdapter(param).Post<bool>(HisRequestUriStore.HisInteractionReason_Delete, ApiConsumer.ApiConsumers.MosConsumer, rowData.ID, param);
                        if (success)
                        {
                            BackendDataWorker.Reset<HIS_INTERACTION_REASON>();
                            FillDataToGridControl();
