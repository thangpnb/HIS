...
namespace HIS.Desktop.Plugins.HisOpenStore
{
    public partial class UCItemOrder : UserControl
    {
        ...
        public UCItemOrder(int widthForm, int heightForm)
        {
            InitializeComponent();
        ...
        private void UCItemOrder_Load(object sender, EventArgs e)
        {
            try
            {
                if (widthParent < 1914 && heightParent < 831)
                {
                    label1.Size = new Size(650 * widthParent / 1914, 40);
                    picOder.Size = new Size(650 * widthParent / 1914, 300 * heightParent / 831);
        ...
        private void SetCaptionByLanguageKey()
        {
            try
            {
                ...
                //this.lblTitle1.Text = Inventec.Common.Resource.Get.Value("UCItemOrder.lblTitle.Text", Resources.ResourceLanguageManager.LanguageResource, LanguageManager.GetCulture());
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Warn(ex);
        ...
        private void picOder_MouseDown(object sender, MouseEventArgs e)
        {
            try
            {
                if (e.Button == MouseButtons.Right)
                {
                    picOder.ContextMenuStrip = new ContextMenuStrip();
